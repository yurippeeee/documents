...
![z 平面と単位円](figures/05_zplane.png)

*図: Z 変換は複素平面全体で定義され、そのうち**単位円上をなぞった値が周波数特性**（DTFT）になる。$`\omega=0`$ が $`z=1`$、$`\omega=\pi`$ が $`z=-1`$ に対応し、円周を一周することが $`\omega`$ を $`0`$ から $`2\pi`$ まで動かすことに等しい。*
...
![ROC の 2 パターン](figures/05_roc.png)
...
![共役極対が生む減衰振動](figures/05_damped_oscillation.png)
