...
> エイリアシング（[01 章 §3](01_離散時間信号とサンプリング.md#サンプリング定理とエイリアシング)）、複素数・共役（[02 章 §0](02_複素指数関数とオイラーの公式.md#複素数の最小限の復習)）、べき級数展開（[02 章 §1](02_複素指数関数とオイラーの公式.md#オイラーの公式の導出)）、等比級数（[02 章 §5](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)）、インパルス応答（[03 章 §3](03_LTIシステムと畳み込み.md#畳み込みの導出-lti-システムはインパルス応答で完全に決まる)）、Z 変換・ROC（[05 章 §1–2](05_Z変換.md#収束領域-roc)）、部分分数展開（[05 章 §4](05_Z変換.md#逆-z-変換-部分分数展開法)）、伝達関数（[06 章 §2](06_伝達関数_極零点_安定性.md#伝達関数の導出)）、極・零点（[06 章 §3](06_伝達関数_極零点_安定性.md#極と零点)）、周波数特性（[06 章 §4](06_伝達関数_極零点_安定性.md#周波数特性の図形的解釈-導出)）、安定性（[06 章 §5](06_伝達関数_極零点_安定性.md#安定性の定理と導出-この章のクライマックス)）、IIR・dB・カットオフ（[07 章](07_IIRフィルタの基礎.md#最小の-iir-1-次フィルタの完全解析)）
...
べき級数 $`\sum_k c_k \Omega^k`$ の係数は $`c_k = f^{(k)}(0)/k!`$（[02 章 §1](02_複素指数関数とオイラーの公式.md#オイラーの公式の導出)）なので、
...
減衰量（dB）を定義通りに書き、§1.0 の $`|H_a(j\Omega)|^2 = 1/[1 + (\Omega/\Omega_c)^{2N}]`$ を代入すると:
...
*図: §1.4・§2.4 の数値例をそのまま描いたもの。灰色が仕様違反ゾーン。同じ仕様(1 dB / 40 dB / 遷移比 2)をバタワースは N=8、チェビシェフ I は N=5、楕円なら N=4 でクリアする(楕円の次数式は本章では導出しないので、この N=4 は結果のみ)。リプルを許すほど(誤差予算を均等配分するほど)次数が下がる。*
...
（先頭の $`T`$ は、サンプリングでスペクトルが $`1/T`$ 倍になる（[01 章 §3](01_離散時間信号とサンプリング.md#サンプリングされた信号のスペクトル-導出)）のを打ち消して、低周波でアナログと同じゲインにするための係数。§5.2 で確認する。）
