...
![IIR 設計の全体フロー](figures/08_design_flow.png)

*図: IIR 設計は「仕様 → 次数決定 → アナログプロトタイプ → （必要なら周波数変換）→ $`s\to z`$ 変換」という一本道。本章はこの各段を順に導出していく。*

> **直感: なぜわざわざアナログ経由なのか。**
> 「デジタルフィルタが欲しいなら最初から z 領域で設計すればいい」と思うのが自然だが、
> z 領域で「この振幅特性に一番近い有理関数」を直接探すのは、係数が分母にも入る有理関数の形を望みの形に近づける問題なので係数について非線形になり、一般に閉じた式では解けない（数値的な最適化はできるが、答えの最適性の保証や設計の見通しが立たない）。
...
> エイリアシング（[01 章 3 節](01_離散時間信号とサンプリング.md#サンプリング定理とエイリアシング)）、複素数・共役（[02 章 0 節](02_複素指数関数とオイラーの公式.md#複素数の最小限の復習)）、べき級数展開（[02 章 1 節](02_複素指数関数とオイラーの公式.md#オイラーの公式の導出)）、等比級数（[02 章 5 節](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)）、インパルス応答（[03 章 3 節](03_LTIシステムと畳み込み.md#畳み込みの導出-lti-システムはインパルス応答で完全に決まる)）、Z 変換・ROC（[05 章 1〜2 節](05_Z変換.md#収束領域-roc)）、部分分数展開（[05 章 4 節](05_Z変換.md#逆-z-変換-部分分数展開法)）、伝達関数（[06 章 2 節](06_伝達関数_極零点_安定性.md#伝達関数の導出)）、極・零点（[06 章 3 節](06_伝達関数_極零点_安定性.md#極と零点)）、周波数特性（[06 章 4 節](06_伝達関数_極零点_安定性.md#周波数特性の図形的解釈-導出)）、安定性（[06 章 5 節](06_伝達関数_極零点_安定性.md#安定性の定理と導出-この章のクライマックス)）、IIR・dB・カットオフ（[07 章](07_IIRフィルタの基礎.md#最小の-iir-1-次フィルタの完全解析)）

## 0. 前提: ラプラス変換と s 平面（最小限）

アナログ側では Z 変換の連続時間版である**ラプラス変換**を使う。アナログフィルタのインパルス応答 $`h_a(t)`$ のラプラス変換がアナログの**伝達関数** $`H_a(s)`$ である:
...
$`u(t)`$ は連続時間の単位ステップ（$`t \geq 0`$ で 1、$`t < 0`$ で 0）で、$`e^{s_0 t}u(t)`$ は「時刻 0 から始まる指数関数」。
...
初期静止（$`x(0)=0`$、因果的）の信号に対し、微分のラプラス変換を部分積分 $`\int f' g = [fg] - \int f g'`$（$`f = x(t)`$、$`g = e^{-st}`$、$`g' = -s e^{-st}`$）で計算:

$$
\int_0^\infty x'(t) e^{-st} dt
= \Big[x(t) e^{-st}\Big]_0^\infty - \int_0^\infty x(t)(-s e^{-st}) dt
= 0 + s\int_0^\infty x(t) e^{-st} dt
= s X(s)
$$
...
因果的で有理な$`H_a(s)`$（単根とする）を部分分数展開（[05 章](05_Z変換.md#逆-z-変換-部分分数展開法)のヘヴィサイド法と同一手順）すると、(a) の対から:

$$
h_a(t) = \sum_i A_i e^{s_i t} u(t), \qquad s_i = \sigma_i + j\Omega_i
$$

連続時間の BIBO 安定条件は$`\int_0^\infty |h_a(t)|dt < \infty`$（[03 章](03_LTIシステムと畳み込み.md#bibo-安定性-iir-理解の要)の離散版と同じ論法: 和を積分に替えるだけ）。
...
全極で$`\sigma_i < 0`$なら有限和で安定。逆にある$`\sigma_1 \geq 0`$なら$`|h_a(t)|`$が 0 に減衰せず発散（[06 章](06_伝達関数_極零点_安定性.md#極と零点)の支配極の議論と同一）。∎
...
$`x = (\Omega/\Omega_c)^2`$とおくと$`|H_a|^2 = \frac{1}{1 + x^N}`$。これを$`x=0`$の周りでべき級数に展開する。
$`|u| < 1`$での等比級数$`\frac{1}{1+u} = 1 - u + u^2 - \cdots`$に$`u = x^N`$を代入すればよい（べき級数展開は一意なので、こうして得た級数がそのままテイラー展開である）:

$$
|H_a(j\Omega)|^2 = 1 - x^N + x^{2N} - \cdots = 1 - \left(\frac{\Omega}{\Omega_c}\right)^{2N} + \cdots
$$

べき級数 $`\sum_k c_k \Omega^k`$ の係数は $`c_k = f^{(k)}(0)/k!`$（[02 章 1 節](02_複素指数関数とオイラーの公式.md#オイラーの公式の導出)）なので、
$`\Omega^1, \Omega^2, \dots, \Omega^{2N-1}`$ の項が存在しない（最初に現れるのが $`\Omega^{2N}`$）ことは、$`\Omega`$に関する$`1`$次から$`2N-1`$次までの導関数が$`\Omega = 0`$ですべて 0 であることと同じである。
...
周波数が 10 倍になるごとに$`-20N`$dB。すなわち **$`-20N`$dB/decade**。周波数 2 倍（1 オクターブ、oct）あたりでは $`-20N\log_{10}2 = -6.02N`$ dB なので **$`-6N`$dB/oct**。次数 1 つで 6dB/oct 稼げる。

![バタワースの次数による変化](figures/08_butterworth_family.png)

*図: 1 節.1〜1.3 の性質を一望したもの。どの次数でも$`\Omega_c`$で −3dB を通り(左)、通過域は平坦なまま、次数を上げるほど阻止域の傾きが 20N dB/dec ずつ急になる(右)。*

### 1.4 設計の核心: 仕様から次数 N を決める式の導出

実際の設計は「$`N`$を先に知っている」のではなく、**仕様から$`N`$を逆算する**ところから始まる。
仕様は普通、次の 2 組 4 つの数 $`(\Omega_p, A_p)`$、$`(\Omega_s, A_s)`$ で与えられる:

- 通過域端$`\Omega_p`$で減衰$`A_p`$dB 以内（例: 1 dB）
- 阻止域端$`\Omega_s`$で減衰$`A_s`$dB 以上（例: 40 dB）

減衰量（dB）を定義通りに書き、1 節.0 の $`|H_a(j\Omega)|^2 = 1/[1 + (\Omega/\Omega_c)^{2N}]`$ を代入すると:

$$
A(\Omega) = -20\log_{10}|H_a(j\Omega)| = -10\log_{10}|H_a(j\Omega)|^2
= -10\log_{10}\frac{1}{1 + (\Omega/\Omega_c)^{2N}}
= 10\log_{10}\left[1 + \left(\frac{\Omega}{\Omega_c}\right)^{2N}\right]
$$

2 つの条件を不等式にする。$`A(\Omega_p) \leq A_p`$ は $`10\log_{10}[1 + (\Omega_p/\Omega_c)^{2N}] \leq A_p`$、両辺を 10 で割って $`10`$ のべきにすると $`1 + (\Omega_p/\Omega_c)^{2N} \leq 10^{A_p/10}`$、1 を移項して下の左式。阻止域側も同様（不等号の向きが逆）:
...
> 1 節.3 で見たように次数 1 つあたり 20 dB/decade 稼げるので、
...
（$`10^{A_s/10} = 10^4`$、$`10^{A_p/10} - 1 = 10^{0.1} - 1 = 1.2589 - 1 = 0.2589`$、$`2\log_{10}(\Omega_s/\Omega_p) = 2\log_{10}2 = 0.6021`$。）

（この同じ仕様をチェビシェフで解くと$`N=5`$で済む。2 節.4 で導出する。）

### 1.5 極の位置の導出

$`|H_a(j\Omega)|^2 = H_a(s)H_a(-s)\big|_{s=j\Omega}`$という関係を使う。
理由: $`|z|^2 = z z^*`$ で、$`H_a`$ の係数が実数なら共役は和と積に分配されて $`H_a(j\Omega)^* = H_a((j\Omega)^*) = H_a(-j\Omega)`$、
よって$`|H_a(j\Omega)|^2 = H_a(j\Omega)H_a(-j\Omega)`$。両辺は $`\Omega`$ の有理関数なので、$`j\Omega`$ を複素変数 $`s`$ に置き換えた $`H_a(s)H_a(-s)`$ という式が $`s`$ 平面全体で意味を持つ（虚軸上の等式を平面全体に広げるこの操作を**解析接続**と呼ぶ。有理関数なら単に $`s`$ を代入するだけである）。
...
> 複素数の$`n`$乗根は常に円周上に等間隔に並ぶ（[02 章](02_複素指数関数とオイラーの公式.md#複素数の極形式と演算)）。
...
漸化式の右辺で最高次を作るのは $`2x T_N`$ の項だけ（引かれる $`T_{N-1}`$ は次数が $`N-1`$ で、$`x^{N+1}`$ には寄与しない）なので、最高次係数は 1 段ごとに 2 倍になり、$`T_1`$ の 1 から始めて **$`T_N`$ の最高次係数は$`2^{N-1}`$**（$`T_N(x) = 2^{N-1}x^N + \cdots`$。帰納法）。
この$`2^{N-1}`$が後で「同じ次数でバタワースより減衰が稼げる」理由として効いてくる（2 節.3）。
...
通過域で最もゲインが落ちるのは $`T_N^2 = 1`$ のときの $`|H_a|^2 = 1/(1 + \varepsilon^2)`$ なので、リプルの深さ（通過域内の最大減衰）を dB で書くと $`A_p = -10\log_{10}\frac{1}{1+\varepsilon^2}`$、すなわち:
...
これは 2 節.1 の漸化式と**同じ漸化式**であり、初期値も$`\cosh(0)=1=T_0(\cosh u)`$、
$`\cosh(u)=T_1(\cosh u)`$で一致する。同じ漸化式・同じ初期値なら全$`N`$で一致する（帰納法）。∎

$`\cosh(Nu) = \frac{e^{Nu}+e^{-Nu}}{2}`$は$`u`$に対して**指数的に**増える。$`\mathrm{arccosh}`$ の式を出しておく: $`u = \mathrm{arccosh}\,x`$ は $`x = \cosh u = (e^u + e^{-u})/2`$ のことで、$`e^u`$ について整理すると $`(e^u)^2 - 2x e^u + 1 = 0`$、解の公式で $`e^u = x + \sqrt{x^2 - 1}`$（$`u \geq 0`$ の方）、よって $`\mathrm{arccosh}\,x = \ln(x + \sqrt{x^2-1})`$。$`x \gg 1`$では$`\sqrt{x^2-1} \approx x`$ なので $`\approx \ln 2x`$。これを使って:

$$
T_N(x) \approx \frac{1}{2}e^{N\ln 2x} = \frac{(2x)^N}{2} = 2^{N-1}x^N
$$

（2 節.1 の最高次係数と一致。検算 ✓）。したがって阻止域の遠方では:
...
*図: 左は$`T_N(x)`$の「変身」—$`x \leq 1`$(青帯)では$`\pm 1`$の間を振動し、$`x=1`$を越えた瞬間に指数的に急増する。右はその結果できる振幅特性 — 通過域は 1 と$`\frac{1}{1+\varepsilon^2}`$の 2 本の線の間を正確に等振幅で往復する(2 節.2 の導出どおり)。*

### 2.4 設計の核心: 仕様から次数 N を決める式の導出

$`\Omega_c = \Omega_p`$（通過域端をリプル帯の端に一致させる）とし、1 節.4 と同じ仕様
（$`\Omega_p`$で$`A_p`$dB 以内、$`\Omega_s`$で$`A_s`$dB 以上）を課す。

通過域条件は 2 節.2 の通り$`\varepsilon = \sqrt{10^{A_p/10}-1}`$で消化済み。阻止域条件は:
...
$`\Omega_s/\Omega_p > 1`$なので 2 節.3 の cosh 表現$`T_N(x) = \cosh(N\mathrm{arccosh}x)`$を使い、
...
**数値例**（1 節.4 と同一仕様:$`A_p=1`$dB,$`A_s=40`$dB,$`\Omega_s/\Omega_p = 2`$）:
...
> バタワース（1 節.4）では減衰が冪$`x^N`$で増えるので、対数をとると$`N`$が線形に出てきて log の比になった。
...
*図: 1 節.4・2 節.4 の数値例をそのまま描いたもの。灰色が仕様違反ゾーン。同じ仕様(1 dB / 40 dB / 遷移比 2)をバタワースは N=8、チェビシェフ I は N=5、楕円なら N=4 でクリアする(楕円の次数式は本章では導出しないので、この N=4 は結果のみ)。リプルを許すほど(誤差予算を均等配分するほど)次数が下がる。*

### 2.5 極の位置の完全導出 — 極は楕円に並ぶ

バタワース（1 節.5）と同じ道具立てで、$`H_a(s)H_a(-s)`$の極を求める。分母 = 0 は:
...
$`\theta = \alpha + j\beta`$（$`\alpha, \beta`$実数）を探す（実数の $`\cos`$ は $`-1`$〜$`1`$ しか取れないが、複素数まで許すと $`\cos\theta`$ は任意の複素数値を取れる。そのような $`\theta`$ が実際に見つかることは、下で $`\alpha, \beta`$ を具体的に解いて確かめる）。このとき定義より$`T_N(v) = \cos(N\theta)`$。
複素引数の$`\cos`$を実部・虚部に分解する（加法定理 +$`\cos(j\beta)=\cosh\beta`$,$`\sin(j\beta)=j\sinh\beta`$、[02 章](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）:
...
> [06 章](06_伝達関数_極零点_安定性.md#伝達関数の導出)で見たとおり、極が周波数軸に近いほどその周波数で鋭いピークを作る——
...
（任意のアナログ伝達関数——$`s`$ の有理関数——は、積分器 $`1/s`$ と定数倍・加算だけを組み合わせたブロック図で実現できる（制御理論の「状態変数表現」の事実。ここでは引用する）ので、積分器の対応を決めればすべて決まる）。
...
（[02 章](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)の$`\sin\theta = \frac{e^{j\theta}-e^{-j\theta}}{2j}`$、$`\cos\theta = \frac{e^{j\theta}+e^{-j\theta}}{2}`$を使用。）
$`s = j\Omega`$と比較して:
...
*図: 左は 3 節.2 の導出の可視化 — s 平面の縦線($`\sigma`$一定の線)が z 平面の円に写り、左半平面($`\sigma<0`$)の線はすべて単位円の内側に収まる。虚軸(黒)がちょうど単位円になる。右は 3 節.3 のワーピング曲線 — 低域では線形対応(緑)とほぼ一致し、高域は$`\omega=\pi`$に向かって圧縮される。*
...
（次数はここでは$`N = 2`$と与える。阻止域仕様から決める場合は 1 節.4 の式を使い、
...
**ステップ 2: アナログプロトタイプ**（1 節.5 で導出済みの 2 次バタワース）
...
**検算 2（安定性、[07 章](07_IIRフィルタの基礎.md#biquad-の安定条件の導出-安定三角形)の安定三角形）**:$`|a_2| = 0.831 < 1`$✓、$`1 + a_1 + a_2 = 0.0157 > 0`$✓、$`1 - a_1 + a_2 = 3.646 > 0`$✓ → 安定
...
*図: 上で求めた係数をそのまま描いたもの。左: プリワーピングの効果で 1 kHz ちょうどで −3.01 dB を通過する。右: 検算 3 の二重零点$`z=-1`$と、低いカットオフ($`f_c \ll f_s`$)ゆえ$`z=1`$の至近に寄った共役極対([09 章](09_実装構造と数値的注意点.md)の数値的注意につながる)。*
...
（先頭の $`T`$ は、サンプリングでスペクトルが $`1/T`$ 倍になる（[01 章 3 節](01_離散時間信号とサンプリング.md#サンプリングされた信号のスペクトル-導出)）のを打ち消して、低周波でアナログと同じゲインにするための係数。5 節.2 で確認する。）

### 5.1 極の写り方の導出

アナログの 1 極$`H_a(s) = \frac{A}{s - s_0}`$のインパルス応答は
$`h_a(t) = A e^{s_0 t} u(t)`$（0 節(a) のラプラス対）。サンプリングすると:

$$
h[n] = T A e^{s_0 T n} u[n] = TA (e^{s_0 T})^n u[n]
$$

これは公比$`e^{s_0 T}`$の指数列なので、Z 変換は（[05 章](05_Z変換.md#逆-z-変換-部分分数展開法)の基本対より）:
...
[01 章](01_離散時間信号とサンプリング.md#エイリアシングの周波数を具体的に導出)で導出したとおり、連続信号$`x_a(t)`$をサンプリングした列$`x_a(nT)`$のスペクトルは
...
カットオフ 1 rad/s に正規化した$`H_{LP}(s)`$（1 節 の式で$`\Omega_c = 1`$としたもの）とする。
...
実係数フィルタの振幅特性は偶関数（$`|H(-j\Omega)| = |H(j\Omega)|`$、[06 章](06_伝達関数_極零点_安定性.md#周波数特性の図形的解釈-導出)）なので:
...
分母は 1 節.5 の LP と同一で、分子だけ$`\Omega_c^2 \to s^2`$に変わる。
...
| インパルス不変法 | 時間波形（インパルス応答）を保存 | エイリアシング（5 節.2）。LPF 限定 | 時間応答の再現が大事な場合 |
