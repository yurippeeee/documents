...
> $`\delta[n]`$・$`u[n]`$・インパルス分解（[01 章 §1](01_離散時間信号とサンプリング.md#重要な基本信号)）、正規化角周波数 $`\omega`$（[01 章 §2](01_離散時間信号とサンプリング.md#正規化周波数)）、複素数・$`j`$・共役 $`z^*`$・偏角 $`\angle z`$（[02 章 §0](02_複素指数関数とオイラーの公式.md#複素数の最小限の復習)）、オイラーの公式と $`\cos, \sin`$ の複素指数表示（[02 章 §1–2](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）、周波数応答 $`H(e^{j\omega})`$・振幅特性・位相特性（[02 章 §4](02_複素指数関数とオイラーの公式.md#なぜ信号処理は複素指数を使うのか-lti-システムの固有関数)）、等比級数（[02 章 §5](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)）、ローパス（[03 章 §1](03_LTIシステムと畳み込み.md#システムとは)）、線形性（[03 章 §2](03_LTIシステムと畳み込み.md#線形性-linearity)）、インパルス応答 $`h[n]`$・畳み込み（[03 章 §3](03_LTIシステムと畳み込み.md#畳み込みの導出-lti-システムはインパルス応答で完全に決まる)）
...
引数を$`\omega`$でなく$`e^{j\omega}`$と書くのは、[05 章 §1](05_Z変換.md#dtft-との関係)の Z 変換$`X(z)`$に$`z = e^{j\omega}`$を代入したものが DTFT に一致するから。つまり **DTFT = Z 変換を単位円上で評価したもの**（先取り）。
...
さらに実係数なら$`|X(e^{-j\omega})| = |X(e^{j\omega})|`$の対称性（§5(c) で導出する）があるため負側も冗長で、結局 $`0 \leq \omega \leq \pi`$ だけ見ればよい。
...
（$`e^{j\theta} - e^{-j\theta} = 2j\sin\theta`$（[02 章 §2](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）を使った。$`m`$が 0 でない整数なら$`\sin(\pi m) = 0`$。）
...
（最後は [01 章 §1](01_離散時間信号とサンプリング.md#重要な基本信号)のインパルス分解の式。）
...
（$`e^{j\omega} + e^{-j\omega} = 2\cos\omega`$（[02 章 §2](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）を使用。）
