...
Z 変換（[05 章](05_Z変換.md)）は DTFT の拡張なので、まずここを固める。

> **この章がやろうとしていること.**
> [02 章](02_複素指数関数とオイラーの公式.md#なぜ信号処理は複素指数を使うのか-lti-システムの固有関数)で「フィルタの働きは**周波数ごとの倍率表**に要約できる」と分かった。
> [03 章](03_LTIシステムと畳み込み.md#この結果の重み)で「フィルタは$`h[n]`$という 1 本の信号で完全に決まる」と分かった。
>
...
> $`\delta[n]`$・$`u[n]`$・インパルス分解（[01 章 1 節](01_離散時間信号とサンプリング.md#重要な基本信号)）、正規化角周波数 $`\omega`$（[01 章 2 節](01_離散時間信号とサンプリング.md#正規化周波数)）、複素数・$`j`$・共役 $`z^*`$・偏角 $`\angle z`$（[02 章 0 節](02_複素指数関数とオイラーの公式.md#複素数の最小限の復習)）、オイラーの公式と $`\cos, \sin`$ の複素指数表示（[02 章 1〜2 節](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）、周波数応答 $`H(e^{j\omega})`$・振幅特性・位相特性（[02 章 4 節](02_複素指数関数とオイラーの公式.md#なぜ信号処理は複素指数を使うのか-lti-システムの固有関数)）、等比級数（[02 章 5 節](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)）、ローパス（[03 章 1 節](03_LTIシステムと畳み込み.md#システムとは)）、線形性（[03 章 2 節](03_LTIシステムと畳み込み.md#線形性-linearity)）、インパルス応答 $`h[n]`$・畳み込み（[03 章 3 節](03_LTIシステムと畳み込み.md#畳み込みの導出-lti-システムはインパルス応答で完全に決まる)）
...
$`X(e^{j\omega})`$は「信号$`x[n]`$の中に、周波数$`\omega`$の回転成分$`e^{j\omega n}`$が**どれだけの量と位相で含まれているか**」を測る**内積**——ベクトルの内積 $`\sum_k a_k b_k`$ と同じ「対応する成分どうしを掛けて足す」演算で、複素数では片方の共役を掛ける（ここでは $`(e^{j\omega n})^* = e^{-j\omega n}`$）——である。
...
> （この「回っているものを 1 周ぶん足すと打ち消し合って 0」は 3 節 で厳密に計算する。）

「合唱の中から特定の人の声だけ聞き取るために、その人のリズムに合わせて首を振る」ようなイメージ。

### 記法について

引数を$`\omega`$でなく$`e^{j\omega}`$と書くのは、[05 章 1 節](05_Z変換.md#dtft-との関係)の Z 変換$`X(z)`$に$`z = e^{j\omega}`$を代入したものが DTFT に一致するから。つまり **DTFT = Z 変換を単位円上で評価したもの**（先取り）。
...
直感: [01 章](01_離散時間信号とサンプリング.md#正規化周波数)で見たとおり、離散信号では$`\omega`$と$`\omega + 2\pi`$は同じ信号を表すので、スペクトルも同じ値になるしかない。
これが「デジタルフィルタの周波数特性は$`0 \leq \omega \leq 2\pi`$を見れば足りる」理由。
さらに実係数なら$`|X(e^{-j\omega})| = |X(e^{j\omega})|`$の対称性（5 節(c) で導出する）があるため負側も冗長で、結局 $`0 \leq \omega \leq \pi`$ だけ見ればよい。
...
ベクトルの内積が 0 になるのと同じ意味で、これを複素指数の**直交性**と呼ぶ。$`m = n - k`$ と置いて、任意の整数$`m`$について次を計算する:
...
（$`e^{j\theta} - e^{-j\theta} = 2j\sin\theta`$（[02 章 2 節](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）を使った。$`m`$が 0 でない整数なら$`\sin(\pi m) = 0`$。）

まとめると（$`\delta[m]`$ は [01 章](01_離散時間信号とサンプリング.md#重要な基本信号)の単位インパルス。$`m = 0`$ で 1、それ以外で 0）:
...
（最後は [01 章 1 節](01_離散時間信号とサンプリング.md#重要な基本信号)のインパルス分解の式。）
...
（[02 章](02_複素指数関数とオイラーの公式.md#なぜ信号処理は複素指数を使うのか-lti-システムの固有関数)の固有関数の議論と完全に整合する）。イコライザーのスライダーそのもの。
...
位相特性（周波数ごとの位相のずれ $`\angle X(e^{j\omega})`$）が $`\omega`$ に比例する直線 $`-\omega n_0`$ になることを**線形位相**と呼ぶ。「遅延 = 線形位相」という事実は位相特性を読むときの基準になる。
...
（共役は和と積に分配でき、実数$`x[n]`$は共役をとっても変わらず、$`(e^{-j\omega n})^* = e^{j\omega n}`$ であるため。）
...
これは公比$`r = a e^{-j\omega}`$の等比級数。$`|r| = |a| < 1`$なので収束し（[02 章](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)の公式）:
...
$$
|X(e^{j\omega})|^2 = X(e^{j\omega}) X(e^{j\omega})^* = \frac{1}{(1 - a e^{-j\omega})(1 - a e^{j\omega})}
= \frac{1}{1 - a(e^{j\omega} + e^{-j\omega}) + a^2}
= \frac{1}{1 - 2a\cos\omega + a^2}
$$

（$`e^{j\omega} + e^{-j\omega} = 2\cos\omega`$（[02 章 2 節](02_複素指数関数とオイラーの公式.md#cos-と-sin-を複素指数で表す-逆向きの公式)）を使用。）

$`0 < a < 1`$の場合の挙動:
- $`\omega = 0`$:$`|X|^2 = \dfrac{1}{(1-a)^2}`$… 最大（分母が最小）
- $`\omega = \pi`$:$`|X|^2 = \dfrac{1}{(1+a)^2}`$… 最小

つまり低い周波数を通し高い周波数を削る**ローパス特性**。$`a`$が 1 に近いほど$`\omega=0`$でのピークが鋭くなる。
（$`-1 < a < 0`$ なら $`\cos\omega`$ の符号が逆に効いて $`\omega = \pi`$ で最大になり、ハイパス特性になる。）
[07 章](07_IIRフィルタの基礎.md)で見る 1 次 IIR ローパスフィルタの周波数特性はまさにこれである。
