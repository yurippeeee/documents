...
引数を$`\omega`$でなく$`e^{j\omega}`$と書くのは、[05 章](05_Z変換.md#逆-z-変換-部分分数展開法)の Z 変換$`X(z)`$に$`z = e^{j\omega}`$を代入したものが DTFT に一致するから。つまり **DTFT = Z 変換を単位円上で評価したもの**（先取り）。
...
（最後は [01 章](01_離散時間信号とサンプリング.md#インパルス列によるサンプリングのモデル化)のインパルス分解の式。）
