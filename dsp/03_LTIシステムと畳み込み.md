...
本シリーズで「フィルタ」と呼んでいるもの——信号を入力すると加工した信号を出力する処理（[01 章](01_離散時間信号とサンプリング.md#重要な基本信号)）——の数学的な正体は **LTI システム（線形時不変システム）** である。
...
例:$`y[n] = \frac{1}{2}(x[n] + x[n-1])`$（隣り合う 2 サンプルの平均 = 簡単なローパスフィルタ。ローパスとは低い周波数を通し高い周波数を削るフィルタのこと）。
...
任意の信号$`x_1, x_2`$と定数$`c_1, c_2`$に対して:
...
> （だから [07 章](07_IIRフィルタの基礎.md)で「係数を動的に変えるときは安定性に注意」という話が出てくる）。
> つまみを固定している限りは時不変。

この 2 つを満たすシステムが **LTI システム**。実用フィルタ——IIR（Infinite Impulse Response、インパルス応答が無限に続くもの）も FIR（Finite Impulse Response、有限で終わるもの）も、3 節 末尾で説明する——はすべて LTI である。
...
1. **入力をインパルスの寄せ集めにバラす**（[01 章 1 節](01_離散時間信号とサンプリング.md#重要な基本信号)でやった分解）
2. **線形性**を使って「バラしてから通す」に切り替える
3. **時不変性**を使って、ずれたインパルスの応答が「ずれた$`h`$」だと言う

これだけで畳み込みの公式が出てくる。では始める。

[01 章 1 節](01_離散時間信号とサンプリング.md#重要な基本信号)で導出した信号の分解を出発点にする:
...
- FIR / IIR という分類は、この$`h[n]`$が有限長か無限長かの分類である（[07 章](07_IIRフィルタの基礎.md)）。
...
和の順序を交換し（各項の絶対値の和が有限——**絶対収束**——なら、2 重和の順序を入れ替えても値が変わらないことが知られている。ここではそれを仮定する）、$`l = m - k`$と変数変換（$`k`$固定で$`m`$が全整数を動けば$`l`$も全整数を動く）:
...
しかも可換性より接続順序を入れ替えても結果は同じ。これが [09 章](09_実装構造と数値的注意点.md)の「biquad 縦続接続」の理論的根拠。
...
（$`\sum_k x[k]h_1[n-k]`$ と $`\sum_k x[k]h_2[n-k]`$ がそれぞれ収束するなら、和は項ごとに 2 つに分けてよい）。これは$`(x*h_1)[n] + (x*h_2)[n]`$。∎
...
$`|x[n]| \leq B_x`$とする。**三角不等式** $`|a + b| \leq |a| + |b|`$（複素数でも成り立つ。三角形の 1 辺は他の 2 辺の和より短い、の意）を繰り返し使うと、有限個の和について $`|\sum a_k| \leq \sum |a_k|`$ が言え、右辺が収束する無限和についても成り立つ（解析学の事実）。よって:
...
（$`x[n] = \operatorname{sgn}(h[-n])`$ に $`n = -k`$ を入れると $`x[-k] = \operatorname{sgn}(h[k])`$。そして $`h[k]\operatorname{sgn}(h[k]) = |h[k]|`$。）
...
- **IIR は$`h[n]`$が無限に続くので、その減衰が十分速いかどうかで安定/不安定が分かれる**。
  例えば$`h[n] = a^n u[n]`$（$`u[n]`$ は [01 章](01_離散時間信号とサンプリング.md#重要な基本信号)の単位ステップ。$`n \geq 0`$ で $`a^n`$、それ以前は 0）なら$`\sum_{n=0}^\infty |a|^n`$は$`|a|<1`$のときだけ収束（[02 章](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)の等比級数）。
  この条件が [06 章](06_伝達関数_極零点_安定性.md)で「極が単位円の内側」という形に一般化される。
