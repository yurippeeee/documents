...
1. **入力をインパルスの寄せ集めにバラす**（[01 章](01_離散時間信号とサンプリング.md#インパルス列によるサンプリングのモデル化)でやった分解）
2. **線形性**を使って「バラしてから通す」に切り替える
3. **時不変性**を使って、ずれたインパルスの応答が「ずれた$`h`$」だと言う

これだけで畳み込みの公式が出てくる。では始める。

[01 章](01_離散時間信号とサンプリング.md#サンプリングされた信号のスペクトル-導出)で導出した信号の分解を出発点にする:
...
- **IIR は$`h[n]`$が無限に続くので、その減衰が十分速いかどうかで安定/不安定が分かれる**。
  例えば$`h[n] = a^n u[n]`$なら$`\sum_{n=0}^\infty |a|^n`$は$`|a|<1`$のときだけ収束（[02 章](02_複素指数関数とオイラーの公式.md#等比級数の和-今後頻繁に使う公式)の等比級数）。
