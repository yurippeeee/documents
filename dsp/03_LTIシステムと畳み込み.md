...
この 2 つを満たすシステムが **LTI システム**。実用フィルタ——IIR（Infinite Impulse Response、インパルス応答が無限に続くもの）も FIR（Finite Impulse Response、有限で終わるもの）も、§3 末尾で説明する——はすべて LTI である。
...
1. **入力をインパルスの寄せ集めにバラす**（[01 章 §1](01_離散時間信号とサンプリング.md#重要な基本信号)でやった分解）
2. **線形性**を使って「バラしてから通す」に切り替える
3. **時不変性**を使って、ずれたインパルスの応答が「ずれた$`h`$」だと言う

これだけで畳み込みの公式が出てくる。では始める。

[01 章 §1](01_離散時間信号とサンプリング.md#重要な基本信号)で導出した信号の分解を出発点にする:
