...
> バイアス（[01 章 4 節](01_電圧電流抵抗.md#分圧回路-最も使う回路)）、逆方向（[04 章 1 節](04_ダイオード.md#ダイオードの動作)）、スイッチ（[05 章 2 節](05_MOSFET.md#3-つの動作領域)）、制御（[06 章 6 節](06_バイポーラ.md#mosfet-との使い分け)）、CMOS（[07 章 2 節](07_オペアンプ内部.md#入力段-差動対)）、低消費電力（[07 章 6 節](07_オペアンプ内部.md#データシートの読み方)）、反転（[08 章 2 節](08_オペアンプ応用.md#基本回路)）、インバータ（[09 章 2 節](09_コンパレータと発振.md#発振回路)）、負荷容量（[09 章 2 節](09_コンパレータと発振.md#発振回路)）
...
[05 章](05_MOSFET.md#p-チャネル-mosfet)で見たとおり、PMOS の移動度は NMOS の 1/2〜1/3 なので、
...
>
> だから——
> - **なまった信号をロジック入力に入れない**（シュミット入力で受ける、[09 章](09_コンパレータと発振.md#シュミットトリガ)）
...
**「入力プルアップ／プルダウン」か「出力ロー」**に設定すること（[19 章](19_GPIO.md)）。
...
| 対策 | 内容 |
|---|---|
| **入力電圧を電源範囲内に保つ** | 直列抵抗、クランプダイオード（[04 章](04_ダイオード.md#ダイオードの種類)） |
| **電源シーケンスを守る** | データシートの規定に従う（[16 章](16_電源設計.md)） |
| **入力電流を制限する** | 多くの IC で「入力ピンあたり ±20 mA」などの規定がある |
| プロセス側の対策 | ガードリング、エピタキシャル基板（設計者側では選べない） |

> **「MCU の電源を切っても動く」現象は、ほぼこれの入口である**（[04 章](04_ダイオード.md#用途-4-クランプ-保護)）。
>
...
>
> **対策: レベルシフタを使う**（[21 章](21_アイソレータ.md#その他のドライバ-レベルシフタ)）。
