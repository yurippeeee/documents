...
> ESR（[02 章 2 節](02_受動素子.md#実際のコンデンサ-esr-と-esl)）、インダクタ（[02 章 7 節](02_受動素子.md#素子の周波数特性まとめ)）、コンデンサ（[02 章 7 節](02_受動素子.md#素子の周波数特性まとめ)）、タンタル（[02 章 3 節](02_受動素子.md#コンデンサの種類)）、ほぼゼロ（[04 章 4 節](04_ダイオード.md#ダイオードの種類)）、ショットキー（[04 章 4 節](04_ダイオード.md#ダイオードの種類)）、スイッチング電源（[05 章](05_MOSFET.md)）、増幅（[05 章 2 節](05_MOSFET.md#3-つの動作領域)）、熱設計（[05 章 9 節](05_MOSFET.md#パワー-mosfet-の実装上の注意)）、制御（[06 章 6 節](06_バイポーラ.md#mosfet-との使い分け)）、飽和（[06 章 3 節](06_バイポーラ.md#動作領域)）、ループ利得（[08 章 1 節](08_オペアンプ応用.md#負帰還の原理)）、反転（[08 章 2 節](08_オペアンプ応用.md#基本回路)）、PLL（[09 章](09_コンパレータと発振.md)）、ループ帯域（[09 章 3 節](09_コンパレータと発振.md#pll-位相同期ループ)）、初期精度（[10 章 1 節](10_リファレンス.md#基準電圧の要件)）、温度ドリフト（[10 章 1 節](10_リファレンス.md#基準電圧の要件)）、誤差アンプ（[10 章 2 節](10_リファレンス.md#バンドギャップリファレンス)）
...
これは [08 章](08_オペアンプ応用.md#非反転増幅)の非反転増幅そのものである。
...
> | **1 W 超** | **スイッチング（[15 章](15_スイッチング電源.md)）** |
...
LDO は負帰還ループなので、**位相余裕が足りないと発振する**（[08 章](08_オペアンプ応用.md#位相余裕)）。
...
>
> そして **MLCC は DC バイアスで容量が半分以下になる**（[02 章](02_受動素子.md#dc-バイアス特性-10-µf-が-2-µf-になる)）。
...
ループ利得は周波数が上がると落ちる（[07 章](07_オペアンプ内部.md#位相補償と-gbw)）。だから:
