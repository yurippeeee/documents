...
> コンデンサ（[02 章 7 節](02_受動素子.md#素子の周波数特性まとめ)）、ノイズ除去（[03 章 6 節](03_過渡応答とフィルタ.md#実務でのフィルタ設計)）、急峻（[03 章 4 節](03_過渡応答とフィルタ.md#フィルタの次数と種類)）、逆方向（[04 章 1 節](04_ダイオード.md#ダイオードの動作)）、オペアンプ（[05 章](05_MOSFET.md)）、スイッチ（[05 章 2 節](05_MOSFET.md#3-つの動作領域)）、制御（[06 章 6 節](06_バイポーラ.md#mosfet-との使い分け)）、ダイナミックレンジ（[08 章 6 節](08_オペアンプ応用.md#単電源での使い方)）、コンパレータ（[09 章](09_コンパレータと発振.md)）、ジッタ（[09 章 3 節](09_コンパレータと発振.md#pll-位相同期ループ)）、熱電対（[10 章 5 節](10_リファレンス.md#温度センサ)）、温度（[11 章 3 節](11_CMOSロジック.md#伝搬遅延)）、直列（[11 章 2 節](11_CMOSロジック.md#複合ゲート)）、最高（[13 章 1 節](13_メモリ.md#メモリの分類-何で覚えているか)）
...
>
> [03 章](03_過渡応答とフィルタ.md#rc-の値の選び方)の熱雑音を思い出す。10 kΩ の抵抗が 1 kHz 帯域で出す雑音は
...
| **精度は容量比で決まる** | IC 内の容量比は非常に正確に作れる（[10 章](10_リファレンス.md#代表的な回路-ブロコウ型)と同じ話） |
...
$`N`$ bit の精度で整定するには（[03 章](03_過渡応答とフィルタ.md#時定数の感覚)）:
...
> **オペアンプの容量負荷になる**（[08 章](08_オペアンプ応用.md#容量性負荷-最も多い発振原因)）。
>
...
[10 章](10_リファレンス.md)で扱ったとおり。要点だけ再掲する。
