...
```
   12 bit の場合、12 回の比較で決まる:

   ステップ 1: VREF/2   と比較 → 上か下か → MSB 確定
   ステップ 2: VREF/4   を足す/引く → 次のビット確定
   ...
   ステップ 12: VREF/4096 → LSB 確定
```
...
```
   ...
   変換期間:
      各容量の下端を VREF か GND に切り替えて、
      コンパレータの入力がゼロに近づくよう探索する
```
...
```
   VIN → [3 bit 変換] → 残差増幅 → [3 bit 変換] → 残差増幅 → ...
           ↓                          ↓
        上位ビット                  次のビット
```
