...
> コンデンサ（[02 章 7 節](02_受動素子.md#素子の周波数特性まとめ)）、ダンピング抵抗（[02 章 6 節](02_受動素子.md#フェライトビーズ-抵抗-として使う部品)）、フェライトビーズ（[02 章 5 節](02_受動素子.md#実際のインダクタ)）、ほぼゼロ（[04 章 4 節](04_ダイオード.md#ダイオードの種類)）、オペアンプ（[05 章](05_MOSFET.md)）、制御（[06 章 6 節](06_バイポーラ.md#mosfet-との使い分け)）、水晶（[09 章 2 節](09_コンパレータと発振.md#発振回路)）、非線形（[10 章 5 節](10_リファレンス.md#温度センサ)）、アナログ入力（[12 章 5 節](12_タイミング.md#消費電力の実務)）、ホットループ（[16 章 6 節](16_電源設計.md#チェックリスト)）、配置（[16 章 3 節](16_電源設計.md#保護)）、後述（[18 章 3 節](18_DAC.md#r-2r-型)）、コネクタ直近（[19 章 7 節](19_GPIO.md#その他の実務知識)）、プルアップ（[19 章 7 節](19_GPIO.md#その他の実務知識)）、外部コネクタ（[19 章 7 節](19_GPIO.md#その他の実務知識)）、I2C（[20 章 1 節](20_シリアル物理層.md#物理層を分ける-4-つの軸)）、コモンモード（[20 章 5 節](20_シリアル物理層.md#can)）、バスハング（[20 章 9 節](20_シリアル物理層.md#まとめ)）、リンギング（[20 章 3 節](20_シリアル物理層.md#spi)）、差動（[20 章 1 節](20_シリアル物理層.md#物理層を分ける-4-つの軸)）、直列抵抗（[20 章 7 節](20_シリアル物理層.md#usb-高速差動)）、配線（[20 章 7 節](20_シリアル物理層.md#usb-高速差動)）、インダクタンス（[22 章 6 節](22_モータドライバ.md#電流検出)）、EMI（[23 章 3 節](23_基板設計.md#反射-リンギングの正体)）、最も効果的（[23 章 5 節](23_基板設計.md#クロストーク)）
...
**対策は明確: ループ面積を減らす。**
これが [23 章](23_基板設計.md#リターン電流-最も重要な概念)の「リターン電流を信号の真下に流す」の意味である。
...
| GND を揺らすもの | 対策 |
|---|---|
| **リターン電流がプレーンの狭い場所に集中** | プレーンを切らない（[23 章](23_基板設計.md#プレーンの切れ目を跨ぐと何が起きるか)） |
| **大電流のスイッチング**（DC-DC、モータ） | ホットループ最小化（[15 章](15_スイッチング電源.md#ホットループ-最重要のレイアウト)・[22 章](22_モータドライバ.md#レイアウトとノイズ)） |
| **IC の同時スイッチング（SSN）** | デカップリング（[16 章](16_電源設計.md#デカップリングの効果を見る)） |
...
> ただし **タイミングが成立する範囲で**（[12 章](12_タイミング.md#出力の立ち上がり時間-スルーレート-制御)）。

### フェライトビーズ

[02 章](02_受動素子.md#フェライトビーズ-抵抗-として使う部品)のとおり、**特定の周波数で抵抗になる部品**である。
...
| 侵入経路 | 対策 |
|---|---|
| **コネクタのピン** | **TVS をコネクタ直近に**（[16 章](16_電源設計.md#esd-と-tvs)） |
...
| **I2C バスハング対策**（[20 章](20_シリアル物理層.md#特有のトラブル)） | 実装しておく |
...
>
> オペアンプの入力段のトランジスタは非線形（[06 章](06_バイポーラ.md#bjt-の式)の指数関数）。
...
| **特定の 1 本だけ鋭いピーク** | 発振（[09 章](09_コンパレータと発振.md#発振の-意図しない-発生)の意図しない発振） |
