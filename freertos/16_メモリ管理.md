...
> TCB（[02 章 §2](02_タスクという抽象.md#タスクとは何か)）、スタック（[02 章 §2](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 §2](02_タスクという抽象.md#タスクとは何か)）、FreeRTOS（[06 章 §4](06_スケジューリング方針.md#優先度の数と表現)）、RAM（[06 章 §4](06_スケジューリング方針.md#優先度の数と表現)）、最も一般的（[08 章 §1](08_ティックとタイムスライス.md#ティック-os-にとっての時間)）、スレッドセーフ（[11 章 §1](11_キュー.md#キューとは何か)）、ブロック（[13 章 §1](13_優先度逆転と優先度継承.md#事件-mars-pathfinder-1997)）、キュー（[14 章 §7](14_タスク通知とイベントグループ.md#ストリームバッファとメッセージバッファ)）
