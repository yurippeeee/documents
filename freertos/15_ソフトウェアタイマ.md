...
> コンテキスト（[01 章 6 節](01_OSがない世界.md#os-が与える-1-つの発明)）、スタック（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、スケジューラ（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、ハードウェア（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、イベントリスト（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、イベント待ち（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、時間待ち（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、FreeRTOS（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）、RAM（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）、削除（[07 章 2 節](07_Readyリストの実装.md#ready-リスト-優先度ごとの配列)）、追加（[07 章 2 節](07_Readyリストの実装.md#ready-リスト-優先度ごとの配列)）、ブロック（[13 章 1 節](13_優先度逆転と優先度継承.md#事件-mars-pathfinder-1997)）、キュー（[14 章 7 節](14_タスク通知とイベントグループ.md#ストリームバッファとメッセージバッファ)）
...
`vTaskStartScheduler()` の中で、**タイマタスクが自動的に生成される**（[03 章](03_FreeRTOSの全体像.md#起動の流れ)）。
...
>
> コールバックに何か書くなら、**このサイズを見直すこと**（[17 章](17_スタックとオーバーフロー.md)）。
...
>
> [14 章](14_タスク通知とイベントグループ.md#xeventgroupsync-ランデブー)で見た `xEventGroupSetBitsFromISR()` も、内部でこれを使っている。
...
タイマも、**期限の昇順にソートされたリスト**で管理される（[10 章](10_ブロックと遅延リスト.md#遅延リスト-起床時刻でソートされたリスト)の遅延リストと同じ発想）。
...
> 特殊な待ち方をする。[10 章](10_ブロックと遅延リスト.md#イベント待ち-2-つのリストに同時登録)で見た「2 つのリストへの同時登録」がそのまま効いている。
>
> - 期限が来たら → 遅延リスト経由で起きる
...
>
> これは [08 章](08_ティックとタイムスライス.md#vtaskdelayuntil-は誤差が累積しない)の `vTaskDelayUntil()` と同じ考え方である。
...
| スケジューラ開始前に `xTimerStart` | コマンドは溜まるが処理されない | 開始後に呼ぶ（[09 章](09_アイドルタスクとフック.md#他のフック)のデーモン起動フックが便利） |
