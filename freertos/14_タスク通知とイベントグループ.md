...
> TCB（[02 章 §2](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 §2](02_タスクという抽象.md#タスクとは何か)）、Ready（[04 章 §1](04_タスクの状態機械.md#4-つの状態)）、イベントリスト（[04 章 §4](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、FreeRTOS（[06 章 §4](06_スケジューリング方針.md#優先度の数と表現)）、追加（[07 章 §2](07_Readyリストの実装.md#ready-リスト-優先度ごとの配列)）、タイムアウト（[10 章 §3](10_ブロックと遅延リスト.md#イベント待ち-2-つのリストに同時登録)）
