...
> TCB（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、スケジューラ（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、移植層（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、Ready（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Suspended（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、イベント待ち（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、ラウンドロビン（[04 章 3 節](04_タスクの状態機械.md#ready-走れるが-順番待ち)）、FreeRTOS（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）
...
遅延リストは起床時刻の昇順にソートされている（[07 章](07_Readyリストの実装.md#種類の挿入)）ので、
