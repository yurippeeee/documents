...
> TCB（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、スケジューラ（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、データ構造（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、Blocked（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Ready（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Suspended（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、イベント待ち（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、ラウンドロビン（[04 章 3 節](04_タスクの状態機械.md#ready-走れるが-順番待ち)）、FreeRTOS（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）
...
これが「イベント待ちのタスクを 2 つのリストから外す」（[04 章](04_タスクの状態機械.md#つの待ち方)）を高速にしている。
...
> そうすれば「先頭だけ見れば、次に起きるタスクが分かる」からである（[10 章](10_ブロックと遅延リスト.md)）。
>
...
> **段数が多いほど、どこに置くべきか分からなくなる**（[06 章](06_スケジューリング方針.md#configmaxpriorities-を大きくするコスト)）。
