...
> **これは誤りである。** 使い分けを間違えると、
> 優先度逆転（[13 章](13_優先度逆転と優先度継承.md#優先度逆転を見る)）が起き、システムが止まる。
>
> FreeRTOS では実装が共通なので、なおさら混同しやすい。
> **何が同じで、何が決定的に違うのか**をはっきりさせる。

> **この章で使う既出の用語（定義は各リンク先）.**
> TCB（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、Blocked（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Ready（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、FreeRTOS（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）、追加（[07 章 2 節](07_Readyリストの実装.md#ready-リスト-優先度ごとの配列)）
...
> 2. **タスク通知**を使う（[14 章](14_タスク通知とイベントグループ.md#タスク通知-vs-キューのコスト)。カウント方式にできる）
>
...
1. **優先度継承**（[13 章](13_優先度逆転と優先度継承.md#優先度継承-freertos-の解決策)）——所有者が誰か分かるから、その優先度を上げられる
...
これが [13 章](13_優先度逆転と優先度継承.md)の主題である。先に結論だけ述べる。
