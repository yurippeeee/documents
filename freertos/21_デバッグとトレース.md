...
> TCB（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、スタック（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、タスク（[02 章 2 節](02_タスクという抽象.md#タスクとは何か)）、スケジューラ（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、移植層（[03 章 2 節](03_FreeRTOSの全体像.md#カーネルの地図)）、Blocked（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Ready（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Running（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、Suspended（[04 章 1 節](04_タスクの状態機械.md#4-つの状態)）、時間待ち（[04 章 4 節](04_タスクの状態機械.md#blocked-条件が整うまで寝る)）、CFS（[06 章 1 節](06_スケジューリング方針.md#唯一の規則)）、FreeRTOS（[06 章 4 節](06_スケジューリング方針.md#優先度の数と表現)）、削除（[07 章 2 節](07_Readyリストの実装.md#ready-リスト-優先度ごとの配列)）、ウォッチドッグ（[09 章 5 節](09_アイドルタスクとフック.md#仕事-3-アイドルフック)）、タイムアウト（[10 章 3 節](10_ブロックと遅延リスト.md#イベント待ち-2-つのリストに同時登録)）、ブロック（[13 章 1 節](13_優先度逆転と優先度継承.md#事件-mars-pathfinder-1997)）、キュー（[14 章 7 節](14_タスク通知とイベントグループ.md#ストリームバッファとメッセージバッファ)）、ティック（[15 章 1 節](15_ソフトウェアタイマ.md#3-つの-タイマ-の違い)）、MPU（[17 章 5 節](17_スタックとオーバーフロー.md#mpu-によるハードウェア検出)）、デッドライン（[20 章 2 節](20_リアルタイム性の解析.md#用語)）、周期（[20 章 2 節](20_リアルタイム性の解析.md#用語)）
...
> - 割り込み優先度の設定が間違っている（[18 章](18_割り込みとの境界.md#割り込み優先度-最も事故が多い設定)）
> - ハンドルが NULL
...
| Stack | **残りスタックの最小値**（ワード。[17 章](17_スタックとオーバーフロー.md#スタック使用量を見る)の HWM） |
| Num | タスク番号（生成順） |

> **`vTaskList()` と `vTaskGetRunTimeStats()` は本番で使ってはいけない.**
>
> どちらも内部で `vTaskSuspendAll()` を呼び、**全タスクを走査する**。
> タスクが 10 個あれば数百マイクロ秒かかる。その間、全タスクの起床が遅れる（[10 章](10_ブロックと遅延リスト.md#xpendingreadylist-スケジューラ停止中の起床)）。
>
> **開発時のデバッグ用**と割り切ること。
...
| `traceTASK_PRIORITY_INHERIT` / `..._DISINHERIT` | **優先度継承の発生**（[13 章](13_優先度逆転と優先度継承.md#優先度継承-freertos-の解決策)） |
...
> **`UNDEFINSTR` が出たら、まずスタックオーバーフローを疑う（[17 章](17_スタックとオーバーフロー.md)）.**
...
| 2 | **スタックオーバーフロー検出を `2` にする**（[17 章](17_スタックとオーバーフロー.md)） |
...
| Tmr Svc が Blocked のまま | **タイマコールバックでブロックした**（[15 章](15_ソフトウェアタイマ.md#コールバックの掟)） |
