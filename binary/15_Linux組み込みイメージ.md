...
> メモリマップ（[01 章 3 節](01_バイナリ解析とは何か.md#ワークフロー-ダンプを手にしてからの-5-手順)）、binwalk（[02 章 3 節](02_道具箱.md#拾う-strings-file-binwalk)）、unblob（[02 章 3 節](02_道具箱.md#拾う-strings-file-binwalk)）、CRC（[03 章 8 節](03_バイト列を読む基本.md#チェックサムと-crc-を見分ける)）、オフセット（[03 章 1 節](03_バイト列を読む基本.md#16-進ダンプの読み方)）、ビッグエンディアン（[03 章 2 節](03_バイト列を読む基本.md#エンディアン-バイトの並び順)）、サイズ（[04 章 7 節](04_ダンプを手に入れる.md#ダンプの健全性を確かめる)）、ブートローダ（[04 章 5 節](04_ダンプを手に入れる.md#ブートローダ-isp-経由)）、ELF（[05 章 1 節](05_ELFファイル.md#elf-とは何か)）、ロードアドレス（[07 章 10 節](07_CortexMのイメージ.md#この章のポイント)）、ファイルシステム（[09 章 1 節](09_ファイルシステム.md#フラッシュとファイルシステムの相性)）、ページ（[10 章 4 節](10_NVM3.md#nvm3-の構造)）、エントリ（[11 章 2 節](11_NVS系ストレージ.md#esp-idf-nvs-の構造)）、バイナリ（[12 章 3 節](12_ログとリングバッファ.md#バイナリログのレコードを読む)）、引数（[12 章 3 節](12_ログとリングバッファ.md#バイナリログのレコードを読む)）、秘密鍵（[13 章 2 節](13_秘密情報の捜索.md#探すべき秘密の一覧)）、圧縮（[14 章 4 節](14_圧縮と暗号化の見分け.md#なぜ見分けが大事か)）
...
これで [01 章](01_バイナリ解析とは何か.md#手順-3-地図を描く)のメモリマップがほぼ完成する。以降は各層を個別に開く。
...
- **`/etc/passwd` `/etc/shadow`**: ユーザとパスワードハッシュ。弱いハッシュ（DES、MD5）や、**全機器共通のハードコードされたパスワード**は脆弱性（[13 章](13_秘密情報の捜索.md#探すべき秘密の一覧)）
- **`/etc/init.d` `/etc/rc*` systemd unit**: 起動時に何が動くか。裏口サービス、telnet の有効化
- **`/etc/ssl` `/etc/*/keys`**: 証明書と秘密鍵（[13 章](13_秘密情報の捜索.md#既知の形式で探す-証明書と鍵)の対象）
- **`/etc/config` `/etc/*.conf`**: ネットワーク・サービス設定、資格情報
- **`/bin` `/sbin` `/usr/bin`**: 独自バイナリ。busybox の版、ベンダ製アプリ → [16 章](16_逆アセンブルの基礎.md)の逆アセンブル対象
