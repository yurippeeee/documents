...
> メモリマップ（[01 章 3 節](01_バイナリ解析とは何か.md#ワークフロー-ダンプを手にしてからの-5-手順)）、オフセット（[03 章 1 節](03_バイト列を読む基本.md#16-進ダンプの読み方)）、チェックサム（[03 章 8 節](03_バイト列を読む基本.md#チェックサムと-crc-を見分ける)）、ブートローダ（[04 章 5 節](04_ダンプを手に入れる.md#ブートローダ-isp-経由)）
...
`srec_info` の出力（"Data: 08000000 - 0807FFFF" のような範囲一覧）は、**メモリマップ作り**（[01 章](01_バイナリ解析とは何か.md#手順-3-地図を描く)）にそのまま使える。
...
マジックは `0x0A324655`（`UF2\n`）で始まる。`uf2conv.py` で `.bin` と相互変換できる。[08 章](08_ブートローダとイメージ形式.md)で触れる。
