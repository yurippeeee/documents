...
> CRC（[03 章 §8](03_バイト列を読む基本.md#チェックサムと-crc-を見分ける)）、オフセット（[03 章 §1](03_バイト列を読む基本.md#16-進ダンプの読み方)）、固定長（[03 章 §5](03_バイト列を読む基本.md#文字列を読む)）、QEMU（[04 章 §6](04_ダンプを手に入れる.md#実機なしで-イメージファイルとエミュレータ)）、マップ（[04 章 §2](04_ダンプを手に入れる.md#デバッグポート-swd-jtag)）、テーブル（[07 章 §6](07_CortexMのイメージ.md#コードとデータを見分ける)）、文字列（[07 章 §6](07_CortexMのイメージ.md#コードとデータを見分ける)）、Storage（[08 章 §2](08_ブートローダとイメージ形式.md#フラッシュ全体のレイアウト)）、NVM3（[10 章 §1](10_NVM3.md#nvm3-とは何か)）、ページ（[10 章 §4](10_NVM3.md#nvm3-の構造)）
