...
> [10 章](10_NVM3.md#nvm3-とは何か)の NVM3 と合わせて「**追記型キー値ストアの共通パターン**」を身につけること。
> 対象が変わっても「エントリ・世代・最新版」という同じ見方が使えることを確認する。

> **この章で使う既出の用語（定義は各リンク先）.**
> CRC（[03 章 8 節](03_バイト列を読む基本.md#チェックサムと-crc-を見分ける)）、オフセット（[03 章 1 節](03_バイト列を読む基本.md#16-進ダンプの読み方)）、固定長（[03 章 5 節](03_バイト列を読む基本.md#文字列を読む)）、QEMU（[04 章 6 節](04_ダンプを手に入れる.md#実機なしで-イメージファイルとエミュレータ)）、マップ（[04 章 2 節](04_ダンプを手に入れる.md#デバッグポート-swd-jtag)）、テーブル（[07 章 6 節](07_CortexMのイメージ.md#コードとデータを見分ける)）、文字列（[07 章 6 節](07_CortexMのイメージ.md#コードとデータを見分ける)）、Storage（[08 章 2 節](08_ブートローダとイメージ形式.md#フラッシュ全体のレイアウト)）、NVM3（[10 章 1 節](10_NVM3.md#nvm3-とは何か)）、ページ（[10 章 4 節](10_NVM3.md#nvm3-の構造)）
...
構造は違うが、**追記型のキー値ストア**という思想は [10 章](10_NVM3.md#追記型の設計-なぜそうなるか)の NVM3 と共通である。まず ESP-IDF NVS を詳しく、次に Zephyr NVS、最後に共通パターンをまとめる。
...
[10 章](10_NVM3.md#nvm3-の構造)の NVM3、この章の ESP-IDF NVS と Zephyr NVS は、細部は違っても**同じ 5 つの部品**を持つ。一度この型を覚えると、初めて見る独自ストレージも読める。
...
5. 値の型に従って中身を解釈する（[03 章](03_バイト列を読む基本.md#バイト列を各型として解釈する)）
...
[10 章](10_NVM3.md#追記型の設計-なぜそうなるか)と同じく、追記型なので**過去の値が残る**。

- 障害解析: あるキーが「いつ、どの値から、どの値へ」変わったかを、世代順に並べれば追える
- セキュリティ: 消したはずの Wi-Fi パスワードや鍵が、古いエントリに平文で残っていないか（[13 章](13_秘密情報の捜索.md)）
