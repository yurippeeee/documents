...
> エントロピー（[01 章 3 節](01_バイナリ解析とは何か.md#ワークフロー-ダンプを手にしてからの-5-手順)）
...
| **取る** | フラッシュから中身を吸い出す | OpenOCD、pyOCD、`flashrom`、ベンダツール（[04 章](04_ダンプを手に入れる.md)） |
...
バイト順のまま読むと 0x20000400 が `00 04 00 20` と見えて分かりにくい（[03 章](03_バイト列を読む基本.md)）。
...
エントロピーの読み方は [03 章](03_バイト列を読む基本.md)と [14 章](14_圧縮と暗号化の見分け.md)で扱う。
...
「公式ツールがあるならまず使う」（[01 章](01_バイナリ解析とは何か.md#手順-4-対象ごとに構造を解く)）。自分で解くのは、ツールがない・壊れていてツールが読めない・ツールの出力の意味を確かめたい、ときである。
...
使い方は [04 章](04_ダンプを手に入れる.md)（ダンプ）と [17 章](17_動的解析.md)（動的解析）で扱う。
...
繰り返し構造を読むには `struct.iter_unpack()`、可変長や条件分岐のある構造には **construct** ライブラリ、可視化と多言語出力が欲しければ **Kaitai Struct** が向く（[19 章](19_自動化とパーサ.md#kaitai-struct-言語非依存-+-可視化)）。
