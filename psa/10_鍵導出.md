...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、操作オブジェクト（[02 章 5 節](02_APIの共通作法.md#操作オブジェクト-分割処理の作法)）、algorithm（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、type（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、usage（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、DER（[04 章 2 節](04_鍵の作成と破棄.md#psaimportkey-バイト列を鍵にする)）、ハッシュ（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）、乱数（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）、CMAC（[06 章 1 節](06_MAC.md#mac-とは何か)）、AES（[07 章 1 節](07_対称暗号.md#対称暗号と-モード)）、カウンタ（[08 章 4 節](08_AEAD.md#ノンスの管理-aead-で唯一難しいところ)）、認証（[09 章 1 節](09_署名と検証.md#署名とは何か-mac-との違い)）

## 1. なぜ鍵を「導出」するのか

暗号の原則に「**1 つの鍵は 1 つの用途に**」がある（[03 章](03_鍵と属性.md#種類-psakeytypet-その鍵はどの暗号のものか)）。
...
| 秘密の入力が `input_key` のとき | 可 | **可**（ただし秘密が鍵 ID で入力された場合、実装によっては `NOT_PERMITTED`。[03 章](03_鍵と属性.md#用途-psakeyusaget-何をしてよいか)の `VERIFY_DERIVATION` 参照） |
...
「鍵の寿命」（[08 章](08_AEAD.md#ノンスの管理-aead-で唯一難しいところ)）の問題を構造的に解く。
