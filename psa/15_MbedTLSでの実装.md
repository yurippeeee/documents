...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、場所（[03 章 7 節](03_鍵と属性.md#寿命-psakeylifetimet-電源を切っても残るか-どこに置くか)）、DER（[04 章 2 節](04_鍵の作成と破棄.md#psaimportkey-バイト列を鍵にする)）、generate（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、import（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、破棄（[04 章 1 節](04_鍵の作成と破棄.md#鍵の一生)）、DRBG（[05 章 2 節](05_乱数とハッシュ.md#psageneraterandom-乱数)）、乱数（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）、MAC（[06 章 1 節](06_MAC.md#mac-とは何か)）、AES（[07 章 1 節](07_対称暗号.md#対称暗号と-モード)）、CTR（[07 章 9 節](07_対称暗号.md#この章のポイント)）、HKDF（[10 章](10_鍵導出.md)）、ECDH（[11 章 1 節](11_鍵合意と公開鍵暗号.md#鍵合意とは何か)）、ITS（[13 章 1 節](13_セキュアストレージ.md#2-つのストレージ-api)）
...
乱数が壊れると ECDSA の秘密鍵が漏れる（[09 章](09_署名と検証.md#アルゴリズムと鍵種別)）。
