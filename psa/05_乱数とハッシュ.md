...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、ビット（[02 章 4 節](02_APIの共通作法.md#出力バッファの流儀-サイズ-と-長さ-のペア)）、操作オブジェクト（[02 章 5 節](02_APIの共通作法.md#操作オブジェクト-分割処理の作法)）、algorithm（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、generate（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、import（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、作成（[04 章 1 節](04_鍵の作成と破棄.md#鍵の一生)）、使用（[04 章 1 節](04_鍵の作成と破棄.md#鍵の一生)）

## 1. この 2 つだけは鍵が要らない

Crypto API のほとんどの操作は鍵 ID を受け取るが、**乱数生成とハッシュだけは鍵を使わない**。
鍵の属性の話をひとまず忘れて、API の作法（[02 章](02_APIの共通作法.md)）そのものを練習するのに最適である。
...
2. `psa_generate_random()` が `PSA_ERROR_INSUFFICIENT_ENTROPY` を返すなら、**エントロピー源が設定されていない**。マイコンでは TRNG（True Random Number Generator、物理現象を使う真性乱数ハードウェア）を Mbed TLS に接続する作業が必要で、これが動かないと `psa_crypto_init()` 自体が失敗する（[15 章](15_MbedTLSでの実装.md)）
...
> ハッシュの照合ではこれは致命的ではないが、MAC の照合（[06 章](06_MAC.md)）では致命的であり、
...
| 署名の作成・検証 | **どちらでも**。`psa_sign_message` は内部で取る。`psa_sign_hash` は自分で取って渡す | [09 章](09_署名と検証.md) |
| MAC | しない（HMAC の内部でハッシュが回る） | [06 章](06_MAC.md) |
| 鍵導出 | しない（HKDF の内部で HMAC が回る） | [10 章](10_鍵導出.md) |
| パスワードの保存 | **ハッシュ単体は不可**。PBKDF2 を使う | [10 章](10_鍵導出.md#pbkdf2-パスワードから鍵を作る) |
...
> 改ざん検出に必要なのは**鍵付きのハッシュ**、すなわち次章の MAC か、[09 章](09_署名と検証.md)の署名である。
