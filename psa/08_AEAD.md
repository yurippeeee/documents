...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、ビット（[02 章 4 節](02_APIの共通作法.md#出力バッファの流儀-サイズ-と-長さ-のペア)）、algorithm（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、type（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、場所（[03 章 7 節](03_鍵と属性.md#寿命-psakeylifetimet-電源を切っても残るか-どこに置くか)）、generate（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、import（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、MAC（[06 章 1 節](06_MAC.md#mac-とは何か)）、AES（[07 章 1 節](07_対称暗号.md#対称暗号と-モード)）、CTR（[07 章 9 節](07_対称暗号.md#この章のポイント)）、対称暗号（[07 章 1 節](07_対称暗号.md#対称暗号と-モード)）

## 1. AEAD とは何か

**AEAD**（Authenticated Encryption with Associated Data、関連データ付き認証暗号）は、
**暗号化（機密性）と MAC（完全性・認証）を 1 つの操作にまとめた**暗号方式である。
[07 章](07_対称暗号.md#なぜ単独で使ってはいけないのか)で述べた「対称暗号を単独で使うな」に対する、標準の答えがこれである。
...
CTR の IV 繰り返し（[07 章](07_対称暗号.md#iv-一度きりの初期値)）よりさらに悪い。
...
対策は、(a) 起動ごとに乱数の「世代番号」をノンスの一部に入れる、(b) カウンタを一定間隔（例: 1000 回ごと）で不揮発ストレージ（[13 章](13_セキュアストレージ.md)）に保存し、起動時に「保存値 + 1000」から始める、のどちらかである。
...
> IoT 機器の一生では届かないことが多いが、「鍵を定期的に更新する」設計（[10 章](10_鍵導出.md)の鍵導出でセッション鍵を作る）が本来の形である。
...
「暗号化しない MAC 付き設定」（[06 章](06_MAC.md#暗号化しないが改ざんは防ぐ-設定データ)）を「AEAD で暗号化した設定」にすると、内容も隠せる。
