...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、ビット（[02 章 4 節](02_APIの共通作法.md#出力バッファの流儀-サイズ-と-長さ-のペア)）、algorithm（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、type（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、usage（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、場所（[03 章 7 節](03_鍵と属性.md#寿命-psakeylifetimet-電源を切っても残るか-どこに置くか)）、import（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、ビッグエンディアン（[04 章 2 節](04_鍵の作成と破棄.md#psaimportkey-バイト列を鍵にする)）、ハッシュ（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）、乱数（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）、MAC（[06 章 1 節](06_MAC.md#mac-とは何か)）、対称暗号（[07 章 1 節](07_対称暗号.md#対称暗号と-モード)）

## 1. 署名とは何か — MAC との違い

**デジタル署名**は、**秘密鍵で作り、公開鍵で検証する**認証値である。
MAC（[06 章](06_MAC.md)）と違い、**検証できる者が作れるとは限らない**。
...
[03 章](03_鍵と属性.md#長さ-bits-生成のときに必要-インポートのときは自動)の表のとおり、`SIGN_HASH` は `SIGN_MESSAGE` を含む（ハッシュに署名できるなら、メッセージをハッシュして署名することもできる）。逆は含まない。
...
| ハッシュの計算 | 呼ぶ側（[05 章](05_乱数とハッシュ.md#ハッシュが使われる場面と-psa-での担当)の分割ハッシュで、大きなイメージでも可） | ライブラリの内部 |
...
> ブートローダの公開鍵は書き換え不能な領域（OTP や TF-M の read-only 鍵）に置く（iotsec 編 [05 章](../iotsec/05_セキュアブート.md)）。
...
また、**サーバが送ってきた任意のデータにそのまま署名してはいけない**——署名対象に「これはチャレンジ応答である」という種別バイトや文脈情報を混ぜる（[14 章](14_アテステーション.md)のアテステーションはこの問題を構造的に解決する）。
...
楕円曲線には「暗号化」の関数がない。楕円曲線で秘密を共有するには**鍵合意**（[11 章](11_鍵合意と公開鍵暗号.md)）を使う。
実際の設計では、RSA-OAEP よりも ECDH + AEAD（[11 章](11_鍵合意と公開鍵暗号.md)の「ハイブリッド」）を選ぶことが多い。
