...
> PSA（[01 章 1 節](01_PSAとは何か.md#psa-は-1-つの物ではない)）、ビット（[02 章 4 節](02_APIの共通作法.md#出力バッファの流儀-サイズ-と-長さ-のペア)）、algorithm（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、type（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、usage（[03 章 11 節](03_鍵と属性.md#この章のポイント)）、generate（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、import（[04 章 10 節](04_鍵の作成と破棄.md#この章のポイント)）、ハッシュ（[05 章 1 節](05_乱数とハッシュ.md#この-2-つだけは鍵が要らない)）
...
[05 章](05_乱数とハッシュ.md#この章のポイント)の最後で述べたように、ハッシュ単体では改ざんを防げない。
...
MAC の鍵は、**「計算」を `SIGN_MESSAGE`、「検証」を `VERIFY_MESSAGE`** として扱う（[03 章](03_鍵と属性.md#用途-psakeyusaget-何をしてよいか)で述べた）。
...
だから用途フラグはあくまで**誤用防止**であり、送信と受信の権限を分けたいなら署名（[09 章](09_署名と検証.md)）を使う。
...
受信したタグを `psa_mac_compute()` で計算し直して `memcmp` で比べるコードを書いてはいけない（[05 章](05_乱数とハッシュ.md#psahashclone-途中まで計算した状態を分岐する)のタイミング攻撃）。
...
ハッシュの分割関数（[05 章](05_乱数とハッシュ.md#ハッシュ-分割関数)）との違いは 2 点だけである。
...
ただし、鍵がそのマイコンから読み出せるなら意味がない——鍵は PSA の永続鍵（[12 章](12_永続鍵とライフタイム.md)）として、できれば TF-M のセキュア側に置く。
...
- 暗号化も要るなら MAC ではなく **AEAD**（[08 章](08_AEAD.md#aead-とは何か)）
