...
>
> [03 章](03_規制と認証.md)で見たとおり、**EU CRA はサポート期間中のセキュリティ更新の提供を義務化した**。
...
> セキュアブート（[02 章 3 節](02_脅威モデリング.md#stride-脅威の分類)）、ハッシュ（[04 章 4 節](04_RootOfTrust.md#鍵の階層)）、eFuse（[06 章 2 節](06_鍵の保管.md#otp-と-efuse)）、完全性（[06 章 6 節](06_鍵の保管.md#外部メモリの保護)）、機密性（[06 章 6 節](06_鍵の保管.md#外部メモリの保護)）、AES（[08 章 8 節](08_暗号エンジンと乱数.md#選定時のチェックポイント)）、レート制限（[09 章 5 節](09_サイドチャネル.md#対策)）、必要（[09 章 6 節](09_サイドチャネル.md#どこまでやるべきか)）、Espressif（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、NXP（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、ライフサイクル状態（[12 章 1 節](12_プロビジョニング.md#何を入れるのか)）
...
| **真正性 (Authenticity)** | 正規のメーカーが作ったものである | **署名検証**（[05 章](05_セキュアブート.md#検証手順)） |
...
> （[04 章](04_RootOfTrust.md#この章のまとめ)の TOCTOU）。**ブートローダが起動のたびに検証するのが本筋である。**
...
| 対策 | 内容 | 注意 |
|---|---|---|
| **外付けフラッシュに Slot 1 を置く** | 内部に Slot 0、外部に Slot 1 | 外部は暗号化する（[06 章](06_鍵の保管.md#外部-dram-の暗号化)） |
| **差分更新（デルタ更新）** | 差分だけ送る。適用時に旧版と合成 | 合成中に電源断すると危険。**中間状態の保護が必要** |
| **圧縮** | 転送量とスロットサイズを減らす | 展開に RAM が要る |
| **単一バンク + リカバリ** | 失敗したらブートローダのリカバリで復旧 | **リカバリ経路が攻撃面になる**（[05 章](05_セキュアブート.md#起動失敗時の設計)） |

> **「A/B 面が取れるフラッシュ容量か」は、SoC 選定の重要な基準である.**
>
> [01 章](01_なぜ難しいのか.md#セキュリティは後付けできない)で述べたとおり、**フラッシュ容量は後から変えられない**。
...
| **TPM の NV カウンタ** | TPM を積んでいる場合（[22 章](22_セキュアエレメント.md)） |
...
>
> 対策:
> - **ブートローダを更新しない設計**にする（機能を最小に保つ = [04 章](04_RootOfTrust.md#immutable-rot-と-updatable-rot)の Immutable RoT に近づける）
...
| **サポート終了 (EOL)** | いつまで更新するかを**購入前に明示する**（[03 章](03_規制と認証.md#英国-psti)の PSTI・CRA 要件） |
...
| **PSA Firmware Update API** | TF-M の一部（[17 章](17_Arm.md#trusted-firmware-m-tf-m)） |
