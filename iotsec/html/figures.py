...
F["s04_keyhier"] = SVG(840, 430, [
    T(420, 26, "鍵の階層 — 何がハードウェアに固定され、何が派生するか", "middle", 12.5, INK, True),
    BOX(260, 50, 320, 62, "HUK（Hardware Unique Key）",
        "機器ごとに固有。CPU から読めない", ALI, ASOFT, INK, 8, 1.6, 12, 9.5),
    BOX(120, 132, 180, 40, "OTP に焼く", None, LIN, PAN, INK, 6, 1.3, 11),
    BOX(330, 132, 180, 40, "PUF から導出", None, LIN, PAN, INK, 6, 1.3, 11),
    W(340, 112, 210, 112, 210, 132, c=MUT), W(420, 112, 420, 132, c=MUT),
    ARR(420, 172, 420, 200, MUT, 1.6), T(432, 192, "KDF で派生", "start", 10, FNT),
    T(478, 224, "派生鍵", "start", 12, SIG, True),
] + sum([[BOX(40 + i * 268, 228, 250, 56, t, s, SIG, SSOFT, INK, 6, 1.3, 11, 9.5)]
         for i, (t, s) in enumerate([
    ("ストレージ暗号鍵", "フラッシュ／NVM の暗号化"),
    ("属性ごとの鍵", "アプリ別・パーティション別"),
    ("アテステーション鍵の保護鍵", "IAK を守る")])], []) + [
    W(420, 200, 165, 200, 165, 228, c=MUT), W(420, 200, 420, 228, c=MUT),
    W(420, 200, 683, 200, 683, 228, c=MUT),
    BOX(40, 316, 380, 46, "ROTPK のハッシュ", "OTP に焼く。署名検証用。公開鍵なので秘密は不要",
        BLU, PAN, INK, 6, 1.3, 11.5, 9.5),
    BOX(440, 316, 360, 46, "機器 ID（IAK / DevID）", "機器固有の秘密鍵と証明書（12 章）",
        BLU, PAN, INK, 6, 1.3, 11.5, 9.5),
    T(420, 302, "別系統", "middle", 10.5, BLU, True),
    T(420, 392, "1 個の HUK から全部を導出する。HUK が読めない限り、派生鍵も再現できない",
      "middle", 11, MUT, True),
    T(420, 412, "だから「HUK を CPU から読めなくする」ことが最優先になる", "middle", 11, ALI, True),
], "鍵は 1 本ずつ管理するのではなく、階層として設計する。根を守れば全体が守られる")
...
F["s11_paths"] = SVG(840, 392, [
    ...
    ("SPI フラッシュ", "SOIC クリップで基板上から読む、または外して読む", "数千円"),
