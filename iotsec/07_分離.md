...
> だが [04 章](04_RootOfTrust.md)で見たとおり、**正規に署名された脆弱なコード**は通ってしまう。
>
...
> セキュアブート（[02 章 3 節](02_脅威モデリング.md#stride-脅威の分類)）、決済（[03 章 5 節](03_規制と認証.md#産業-自動車-医療)）
...
> は品種ごとに違う。
>
> **リファレンスマニュアルで必ず確認すること。** [18 章](18_ST.md)〜[21 章](21_その他ベンダ.md)で各社の実装を見る。
...
TF-M では、通常のアプリケーションが動く Non-secure 側を **NSPE**
（Non-secure Processing Environment）、TF-M 本体が動く Secure 側を
**SPE**（Secure Processing Environment）と呼ぶ。
...
| **Initial Attestation** | アテステーショントークンを生成する（[04 章](04_RootOfTrust.md#アテステーション-証明)） |
| **Firmware Update** | セキュア OTA の制御（[13 章](13_セキュアOTA.md)） |
...
> ESP-TEE を提供している（[20 章](20_Espressif.md#esp-tee-新しい取り組み)）。
...
| **サイドチャネル** | Secure の処理を、Non-secure から消費電力やキャッシュで観測する | DPA 対策、キャッシュ分離（[09 章](09_サイドチャネル.md)） |
| **フォールト** | 越境チェックをグリッチで飛ばす | 冗長チェック（[10 章](10_フォールトインジェクション.md)） |
