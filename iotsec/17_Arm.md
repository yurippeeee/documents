...
> [18 章](18_ST.md)〜[21 章](21_その他ベンダ.md)の各社の説明が「その会社流の実装」として整理できる。**

> **注意:** 以下は執筆時点の公開情報に基づく概観である。
> 具体的な対応状況は、**必ず対象品種のデータシートと Arm の公式資料で確認すること。**

> **この章で使う既出の用語（定義は各リンク先）.**
> セキュアブート（[02 章 3 節](02_脅威モデリング.md#stride-脅威の分類)）、PSA-RoT（[04 章 2 節](04_RootOfTrust.md#4-つの-root-of-trust)）、物理攻撃（[04 章 6 節](04_RootOfTrust.md#rot-が守れないもの)）、不可（[06 章 1 節](06_鍵の保管.md#鍵はどこに置けるか)）、GTZC（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、NSPE（[07 章 4 節](07_分離.md#tf-m-trusted-firmware-m)）、Non-secure（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、SPU（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、Secure（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、TrustZone（[07 章 2 節](07_分離.md#分離の-3-つのレベル)）、ポインタ検証（[07 章 8 節](07_分離.md#分離だけでは足りないもの)）、両方（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、CryptoCell（[08 章 5 節](08_暗号エンジンと乱数.md#暗号アクセラレータ)）、キャッシュ（[09 章 1 節](09_サイドチャネル.md#サイドチャネルとは何か)）、必要（[09 章 6 節](09_サイドチャネル.md#どこまでやるべきか)）、消費電力（[09 章 1 節](09_サイドチャネル.md#サイドチャネルとは何か)）、ECDSA（[10 章 2 節](10_フォールトインジェクション.md#何が狙われるか)）、Nordic（[11 章 4 節](11_ファームウェア抽出.md#経路-3-swd-jtag)）、NXP（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、Renesas（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、認証付きデバッグ（[14 章 3 節](14_ライフサイクル.md#rma-返ってきた機器をどう調べるか)）、開発（[14 章 5 節](14_ライフサイクル.md#開発と量産の切り分け)）、Rust（[15 章 3 節](15_ソフトウェア防御.md#防御-言語とライブラリ)）、要件（[15 章 8 節](15_ソフトウェア防御.md#セキュア開発ライフサイクル-sdl)）、RAM（[16 章 3 節](16_通信セキュリティ.md#リソース制約への対応)）、フラッシュ（[16 章 3 節](16_通信セキュリティ.md#リソース制約への対応)）、ポート（[16 章 6 節](16_通信セキュリティ.md#mqtt-の設定)）
...
| 層 | 名称 | 内容 |
|---|---|---|
| **アーキテクチャ** | **TrustZone-M**（Armv8-M） | Secure / Non-secure の分離（[07 章](07_分離.md#mpu-による分離-trustzone-がない場合)） |
| **アーキテクチャ** | **TrustZone-A**（Armv7-A/v8-A） | アプリケーションプロセッサ向けの分離 |
| **アーキテクチャ** | **PACBTI**（Armv8.1-M） | 制御フロー保護（[15 章](15_ソフトウェア防御.md#armv8.1-m-の-pacbti)） |
...
| **仕様** | **ADAC** | 認証付きデバッグ（[11 章](11_ファームウェア抽出.md#認証付きデバッグ)） |
| **認証** | **PSA Certified** | 第三者評価プログラム（[03 章](03_規制と認証.md#psa-certified-arm-主導)） |
...
> 実際の耐性は、**PSA Certified Level 3 や CC の取得状況**で判断すること（[03 章](03_規制と認証.md#psa-certified-arm-主導)）。

### PACBTI（Armv8.1-M）

[15 章](15_ソフトウェア防御.md#armv8.1-m-の-pacbti)で触れた制御フロー保護である。
...
[07 章](07_分離.md)で概念を扱ったので、ここでは実装に関わる点を補う。
...
| 段階 | 内容 |
|---|---|
| **Analyze** | 脅威モデル（Threat Model and Security Analysis, TMSA）を作る（[02 章](02_脅威モデリング.md)） |
| **Architect** | セキュリティ要件を満たすアーキテクチャを設計する |
| **Implement** | **TF-M** などで実装する |
| **Certify** | **PSA Certified** で第三者評価を受ける（[03 章](03_規制と認証.md#psa-certified-arm-主導)） |
...
| 2 | セキュリティライフサイクル（[14 章](14_ライフサイクル.md)） |
| 3 | アテステーション（[04 章](04_RootOfTrust.md#アテステーション-証明)） |
| 4 | セキュアブート（[05 章](05_セキュアブート.md)） |
| 5 | セキュア更新（[13 章](13_セキュアOTA.md)） |
| 6 | アンチロールバック（[13 章](13_セキュアOTA.md)） |
| 7 | 分離（[07 章](07_分離.md)） |
| 8 | インタラクション（分離間の安全な通信） |
| 9 | 暗号サービス（[08 章](08_暗号エンジンと乱数.md)） |
| 10 | セキュアストレージ（[06 章](06_鍵の保管.md)） |
...
**PSA-RoT と ARoT を分けるのは、[07 章](07_分離.md#分離だけでは足りないもの)の「Secure 側を小さく保つ」の実装である。**
...
| **BL1 / BL2**（MCUboot ベース） | セキュアブート（[05 章](05_セキュアブート.md#測定ブート-measured-boot-との違い)） |
...
| **自作** | **強く非推奨**。TrustZone の落とし穴が多すぎる（[07 章](07_分離.md#mpu-による分離-trustzone-がない場合)） |
...
| 機能 | 内容 |
|---|---|
| **TrustZone-A** | Secure World / Normal World。EL3 のモニタが切り替える（[07 章](07_分離.md#cortex-a-クラスの分離)） |
...
| **PSA Certified** | **Level 1 / 2 / 3 のどれを取得しているか**（[03 章](03_規制と認証.md#psa-certified-arm-主導)） |
| **PSA Functional API 認証** | Crypto API などの適合試験に通っているか |
| **暗号 IP** | CryptoCell か、ベンダ独自か。DPA 対策の有無 |
| **ADAC** | 認証付きデバッグに対応しているか（[11 章](11_ファームウェア抽出.md#認証付きデバッグ)・[14 章](14_ライフサイクル.md#認証付きデバッグとの組み合わせ)） |
