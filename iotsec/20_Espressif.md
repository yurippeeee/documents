...
> セキュアブート（[02 章 3 節](02_脅威モデリング.md#stride-脅威の分類)）、物理攻撃（[04 章 6 節](04_RootOfTrust.md#rot-が守れないもの)）、署名検証（[04 章 4 節](04_RootOfTrust.md#鍵の階層)）、RSA-3072（[05 章 3 節](05_セキュアブート.md#署名アルゴリズムの選択)）、eFuse（[06 章 2 節](06_鍵の保管.md#otp-と-efuse)）、不可（[06 章 1 節](06_鍵の保管.md#鍵はどこに置けるか)）、完全性（[06 章 6 節](06_鍵の保管.md#外部メモリの保護)）、機密性（[06 章 6 節](06_鍵の保管.md#外部メモリの保護)）、起動時間（[06 章 3 節](06_鍵の保管.md#puf-physically-unclonable-function)）、Non-secure（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、Secure（[07 章 3 節](07_分離.md#arm-trustzone-m-armv8-m)）、サイドチャネル（[07 章 8 節](07_分離.md#分離だけでは足りないもの)）、AES（[08 章 8 節](08_暗号エンジンと乱数.md#選定時のチェックポイント)）、Nordic（[11 章 4 節](11_ファームウェア抽出.md#経路-3-swd-jtag)）、Espressif（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、Renesas（[12 章 3 節](12_プロビジョニング.md#工場側の保護)）、量産（[14 章 5 節](14_ライフサイクル.md#開発と量産の切り分け)）、開発（[14 章 5 節](14_ライフサイクル.md#開発と量産の切り分け)）、運用（[15 章 8 節](15_ソフトウェア防御.md#セキュア開発ライフサイクル-sdl)）、RAM（[16 章 3 節](16_通信セキュリティ.md#リソース制約への対応)）、フラッシュ（[16 章 3 節](16_通信セキュリティ.md#リソース制約への対応)）、TrustZone-M（[17 章 1 節](17_Arm.md#arm-のセキュリティ製品群の地図)）、TEE（[19 章 8 節](19_NXP.md#i.mx-cortex-a-のセキュリティ)）
...
ESP32 のセキュリティ設定は、**すべて eFuse で制御される**（[06 章](06_鍵の保管.md#otp-と-efuse)）。
...
> **セキュアブートのダイジェストが 3 個持てるのは重要である**（[12 章](12_プロビジョニング.md#rotpk-の切り替え)）。
>
...
> **これが [11 章](11_ファームウェア抽出.md#出荷前チェックリスト)のチェックリストそのものである.**
>
> **Secure Boot を有効にしても、
...
**外付け SPI フラッシュの内容を暗号化する**（[06 章](06_鍵の保管.md#内部フラッシュの読み出し保護)）。
...
> だからこそ、**量産前に Release モードで全機能を検証すること**（[14 章](14_ライフサイクル.md#開発と量産の切り分け)）。
...
> | Flash Encryption のみ | **改ざんを検出できない**（[06 章](06_鍵の保管.md#オンザフライ暗号)の「暗号化 ≠ 完全性」） |
> | **両方** | 実用上の防御が成立する |
...
>
> [06 章](06_鍵の保管.md#otp-の使い方の設計)で述べた「鍵の不可視化」を、
> **追加のチップなしで実現できる**。
>
...
> 専用のセキュアエレメントには及ばない**ことを理解しておくこと（[22 章](22_セキュアエレメント.md)）。
...
> **RISC-V には標準の TrustZone 相当がない**（[07 章](07_分離.md#mpu-による分離-trustzone-がない場合)）ので、
...
| 3 | **署名鍵は HSM で管理されているか**（[12 章](12_プロビジョニング.md#何を-id-にするか)） | 運用 |
...
| 12 | **OTA が署名検証つきか、ロールバック防止が有効か**（[13 章](13_セキュアOTA.md)） | menuconfig `Enable app rollback support` |
| 13 | **TLS の証明書検証が有効か**（[16 章](16_通信セキュリティ.md#失敗-1-証明書検証の無効化)） | コード |
| 14 | **ファームウェアに秘密が埋まっていないか**（[11 章](11_ファームウェア抽出.md#経路-1-公開ファームウェアと-ota)） | CI の `strings` チェック |
...
| **Espressif の事前プロビジョニング** | モジュールに鍵と証明書を事前注入するサービスがある（[12 章](12_プロビジョニング.md#セキュアプロビジョニングの支援機能)） |
...
| 忘れがちな設定 | **UART ダウンロードモードと JTAG の無効化**（[11 章](11_ファームウェア抽出.md#経路-5-isp-ブートローダモード)） |
