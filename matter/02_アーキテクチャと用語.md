...
> Matter（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、コミッショニング（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）
...
>    Matter メッセージを運ぶ。そして**コミッショニング時にしか使わない**（[05 章](05_WiFiとBLE.md)）。
...
> DAC 証明書（[12 章](12_デバイス認証と証明書.md)）の VID/PID と、証明書のシリアル番号が担う。
...
> 独自クラスタは付加機能に留めるのが定石である（[23 章](23_落とし穴と設計指針.md)）。
...
> ネットワーク上に Fabric ID そのものを晒さないための工夫でもある（[06 章](06_探索とメッセージング.md)）。
...
> **フラッシュと RAM を消費する**。設計時に必ず見積もること（[17 章](17_デバイス実装の実際.md)）。
> 実際の製品では 5〜10 程度が多い。
...
| 11 | | 購読していれば `OnOff` 属性の変化が **Report** として通知される（[10 章](10_サブスクリプション.md)） |
...
> [21 章](21_デバッグとテスト.md)のデバッグはこの分解が土台になる。
...
詳細と実際のエンコード例は [17 章](17_デバイス実装の実際.md)で扱う（コーデックのデモも用意している）。
...
> Device Library は「実装漏れがないか」の最終チェックリストになる（[22 章](22_認証と製品化.md)）。
