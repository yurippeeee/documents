...
> コミッショニング（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、ネットワーク（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、相互運用性（[01 章 2 節](01_Matterとは何か.md#matter-の設計目標)）、ACL（[02 章 4 節](02_アーキテクチャと用語.md#fabric-と信頼の構造)）、Administrator（[02 章 5 節](02_アーキテクチャと用語.md#ロールの分類)）、Attribute（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Client（[02 章 5 節](02_アーキテクチャと用語.md#ロールの分類)）、Cluster（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Command（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Endpoint（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Wi-Fi（[04 章 9 節](04_Thread.md#thread-を選ぶべきか)）、Group（[06 章 4 節](06_探索とメッセージング.md#セッションの種類)）、FeatureMap（[07 章 2 節](07_データモデルの構造.md#attribute-属性)）、ミレッド（[07 章 6 節](07_データモデルの構造.md#データ型)）
...
| `0x001F` | **Access Control** | ACL（[14 章](14_運用時のセキュリティ.md)） | 必須 |
| `0x003E` | **Operational Credentials** | 証明書・Fabric 管理（[12 章](12_デバイス認証と証明書.md)〜[14 章](14_運用時のセキュリティ.md)） | 必須 |
...
| `0x002A` | **OTA Software Update Requestor** | 更新を受ける側（[18 章](18_OTA.md)） | OTA 対応なら |
...
| `0x0046` | **ICD Management** | 間欠動作の管理（[17 章](17_デバイス実装の実際.md#icd-management-クラスタ-0x0046)） | ICD なら |
| `0x003C` | Administrator Commissioning | Commissioning Window の開閉（[14 章](14_運用時のセキュリティ.md)） | 必須 |
...
> OTA では「Provider の版がこれより大きいか」で更新を判断する（[18 章](18_OTA.md)）。
...
| `0x001E` | **Binding** | 機器どうしの直接連携（[07 章](07_データモデルの構造.md#binding-機器どうしの直接連携)） |
...
>
> **センサー系はどれも Nullable** である（[07 章](07_データモデルの構造.md#nullable)）。
...
| `0x000E` | **Aggregator** | ブリッジの親（[19 章](19_ブリッジ.md#aggregator-0x000e)） |
| `0x0013` | **Bridged Node** | ブリッジされた機器（[19 章](19_ブリッジ.md#bridged-node-0x0013)） |
