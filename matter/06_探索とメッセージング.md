...
>
> この章は [21 章](21_デバッグとテスト.md)のデバッグの土台になる。

> **この章で使う既出の用語（定義は各リンク先）.**
> Matter（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、コミッショニング（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、セキュリティ（[01 章 2 節](01_Matterとは何か.md#matter-の設計目標)）、ネットワーク（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、ACL（[02 章 4 節](02_アーキテクチャと用語.md#fabric-と信頼の構造)）、IPv6（[02 章 1 節](02_アーキテクチャと用語.md#レイヤ構造)）、Node（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、メッセージング（[02 章 1 節](02_アーキテクチャと用語.md#レイヤ構造)）、マルチキャスト（[03 章 3 節](03_IPv6の基礎.md#アドレスの種類-ここを取り違えると詰まる)）、Channel（[04 章 4 節](04_Thread.md#operational-dataset-thread-の-ネットワーク設定)）、Router（[04 章 2 節](04_Thread.md#ノードの役割)）、Wi-Fi（[04 章 9 節](04_Thread.md#thread-を選ぶべきか)）、必要（[05 章 5 節](05_WiFiとBLE.md#ネットワークの選び方)）
...
| 部分 | 内容 |
|---|---|
| Compressed Fabric ID | Root Public Key と Fabric ID から HKDF で導出した 64 bit（[02 章](02_アーキテクチャと用語.md#fabric-とは何か)） |
...
> **手動ペアリングコード（[13 章](13_コミッショニング.md#手動ペアリングコード)）には Short Discriminator しか入らない.**
...
Thread デバイスは mDNS のマルチキャストを常時受信できない（[04 章](04_Thread.md#srp-service-registration-protocol)）。
...
| **PASE** | パスコード（SPAKE2+） | **コミッショニング時のみ**（[13 章](13_コミッショニング.md)） |
| **CASE** | 証明書（SIGMA） | **運用時**（[14 章](14_運用時のセキュリティ.md)） |
