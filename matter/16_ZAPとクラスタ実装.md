...
> Matter（[01 章 §3](01_Matterとは何か.md#matter-は何であって-何でないか)）、コミッショニング（[01 章 §3](01_Matterとは何か.md#matter-は何であって-何でないか)）、ネットワーク（[01 章 §3](01_Matterとは何か.md#matter-は何であって-何でないか)）、Attribute（[02 章 §2](02_アーキテクチャと用語.md#用語の地図)）、Client（[02 章 §5](02_アーキテクチャと用語.md#ロールの分類)）、Cluster（[02 章 §2](02_アーキテクチャと用語.md#用語の地図)）、Command（[02 章 §2](02_アーキテクチャと用語.md#用語の地図)）、Endpoint（[02 章 §2](02_アーキテクチャと用語.md#用語の地図)）、Report（[02 章 §6](02_アーキテクチャと用語.md#1-つの操作を最後まで追う)）、Server（[02 章 §5](02_アーキテクチャと用語.md#ロールの分類)）、インタラクションモデル（[02 章 §1](02_アーキテクチャと用語.md#レイヤ構造)）、必要（[05 章 §5](05_WiFiとBLE.md#ネットワークの選び方)）、FeatureMap（[07 章 §2](07_データモデルの構造.md#attribute-属性)）、CurrentLevel（[08 章 §2](08_クラスタとデバイスタイプ.md#汎用アプリケーションクラスタ)）、Identify（[08 章 §2](08_クラスタとデバイスタイプ.md#汎用アプリケーションクラスタ)）、NodeLabel（[08 章 §1](08_クラスタとデバイスタイプ.md#ユーティリティクラスタ-endpoint-0)）、OnOff（[08 章 §2](08_クラスタとデバイスタイプ.md#汎用アプリケーションクラスタ)）、Path（[09 章 §2](09_インタラクションモデル.md#path-何を指すか)）、Read（[09 章 §1](09_インタラクションモデル.md#4-つのインタラクション)）
...
> ここで「対応する」と宣言した機能は必ず実装すること（[08 章](08_クラスタとデバイスタイプ.md#実装の進め方)）。
...
> 値が変わったときは自分で `MatterReportingAttributeChangeCallback` を呼ぶ必要がある（[10 章](10_サブスクリプション.md#デバイス側の実装ポイント)）。
...
ブリッジ（[19 章](19_ブリッジ.md#ブリッジとは)）などでは、実行時に Endpoint を増やす必要がある。
...
どうしても必要な場合（[07 章](07_データモデルの構造.md#binding-機器どうしの直接連携)の警告を読んだうえで）。
...
```
1. Device Library で必須クラスタを確認（[08 章](08_クラスタとデバイスタイプ.md)）
       ↓
...
| 報告トリガの呼び忘れ | 「読めば正しいが通知が来ない」（[10 章](10_サブスクリプション.md#デバッグ)） |
| ハードウェア → Matter の反映漏れ | 物理操作がアプリに反映されない |
| External 属性で通知を出さない | 同上 |
| 属性の永続化を忘れる | 電源断で設定が消える |
| コールバックの中でブロックする | スタック全体が止まる（[09 章](09_インタラクションモデル.md#実装上の制約)） |
