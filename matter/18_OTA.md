...
> Matter（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、アプリケーション層（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、コミッショニング（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、セキュリティ（[01 章 2 節](01_Matterとは何か.md#matter-の設計目標)）、ネットワーク（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、相互運用性（[01 章 2 節](01_Matterとは何か.md#matter-の設計目標)）、ACL（[02 章 4 節](02_アーキテクチャと用語.md#fabric-と信頼の構造)）、Wi-Fi（[04 章 9 節](04_Thread.md#thread-を選ぶべきか)）、必要（[05 章 5 節](05_WiFiとBLE.md#ネットワークの選び方)）、Exchange（[06 章 5 節](06_探索とメッセージング.md#メッセージの構造-概略)）、指数バックオフ（[06 章 3 節](06_探索とメッセージング.md#mrp-メッセージ信頼性プロトコル)）、ProductID（[08 章 1 節](08_クラスタとデバイスタイプ.md#ユーティリティクラスタ-endpoint-0)）、VendorID（[08 章 1 節](08_クラスタとデバイスタイプ.md#ユーティリティクラスタ-endpoint-0)）、ハッシュ（[11 章 2 節](11_セキュリティの全体像.md#使われている暗号プリミティブ)）、改竄（[11 章 1 節](11_セキュリティの全体像.md#脅威モデル-何を想定しているか)）、署名（[11 章 2 節](11_セキュリティの全体像.md#使われている暗号プリミティブ)）、DCL（[12 章 11 節](12_デバイス認証と証明書.md#まとめ)）、メーカー（[12 章 3 節](12_デバイス認証と証明書.md#cd-certification-declaration)）
...
> （上の例の 2 番目のエントリ）。
> ここを忘れると `UNSUPPORTED_ACCESS` で止まる（[14 章](14_運用時のセキュリティ.md#落とし穴)）。
...
| **バージョン管理** | `SoftwareVersion` は単調増加の整数（[08 章](08_クラスタとデバイスタイプ.md#basic-information-の主な属性)） |
| **DCL への登録** | OTA 情報を DCL に載せる方式もある（[12 章](12_デバイス認証と証明書.md#dcl-distributed-compliance-ledger)） |
...
> 認証取得（[22 章](22_認証と製品化.md)）とセットで確認すべき事項である。

## 8. 落とし穴

| 落とし穴 | 対処 |
|---|---|
| **Flash 容量が足りない** | 設計時に 2 面分を確保（[17 章](17_デバイス実装の実際.md#落とし穴)） |
