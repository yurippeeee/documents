...
> Matter（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、コミッショニング（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、ネットワーク（[01 章 3 節](01_Matterとは何か.md#matter-は何であって-何でないか)）、Attribute（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Cluster（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、Command（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、MRP（[02 章 1 節](02_アーキテクチャと用語.md#レイヤ構造)）、Node（[02 章 2 節](02_アーキテクチャと用語.md#用語の地図)）、インタラクションモデル（[02 章 1 節](02_アーキテクチャと用語.md#レイヤ構造)）、リンク（[02 章 1 節](02_アーキテクチャと用語.md#レイヤ構造)）、Channel（[04 章 4 節](04_Thread.md#operational-dataset-thread-の-ネットワーク設定)）、BLE（[05 章 1 節](05_WiFiとBLE.md#matter-がサポートするリンク層)）、必要（[05 章 5 節](05_WiFiとBLE.md#ネットワークの選び方)）、Exchange（[06 章 5 節](06_探索とメッセージング.md#メッセージの構造-概略)）、UDP（[06 章 3 節](06_探索とメッセージング.md#mrp-メッセージ信頼性プロトコル)）、OnOff（[08 章 2 節](08_クラスタとデバイスタイプ.md#汎用アプリケーションクラスタ)）、Toggle（[08 章 2 節](08_クラスタとデバイスタイプ.md#汎用アプリケーションクラスタ)）、Read（[09 章 1 節](09_インタラクションモデル.md#4-つのインタラクション)）、ワイルドカード（[09 章 11 節](09_インタラクションモデル.md#まとめ)）、証明書（[11 章 5 節](11_セキュリティの全体像.md#セッション確立の-2-つの道)）、バージョン管理（[14 章 3 節](14_運用時のセキュリティ.md#acl-アクセス制御リスト)）
...
│   ├── lock-app/            ← ドアロック
│   ├── bridge-app/          ← ブリッジ（[19 章](19_ブリッジ.md)）
│   ├── ota-provider-app/    ← OTA 配布側（[18 章](18_OTA.md)）
│   ├── ota-requestor-app/   ← OTA 受信側
│   ├── chip-tool/           ← **CLI コントローラ（必携）**
│   └── ...
├── zzz_generated/           ← ZAP が生成したコード（[16 章](16_ZAPとクラスタ実装.md)）
...
> **自動テストを書くなら Python 側が圧倒的に楽である**（[21 章](21_デバッグとテスト.md)）。
...
> **「開発中に SDK を上げたら仕様バージョンが変わっていた」は事故になる**（[22 章](22_認証と製品化.md)）。
...
| 生成コードを手で編集する | ZAP で再生成すると消える（[16 章](16_ZAPとクラスタ実装.md)） |
